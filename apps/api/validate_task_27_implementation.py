...
def validate_migration_file() -> Dict[str, Any]:
    ...
    migration_file = (
        ...
        / "alembic"
        / "versions"
        / "20250817_1800-task_27_global_constraints_performance_indexes.py"
    )
    ...
    if unique_found >= 6:  # Most constraints should be present
        ...
        validation_results["summary"].append(
            f"[OK] Unique constraints implemented ({unique_found}/7)"
        )
    ...
    if check_found >= 3:
        ...
        validation_results["summary"].append(
            f"[OK] Check constraints implemented ({check_found}/4)"
        )
    ...
    if perf_found >= 5:
        ...
        validation_results["summary"].append(
            f"[OK] Performance indexes implemented ({perf_found}/7)"
        )
    ...
    jsonb_found = sum(
        ...
        if f"gin_{table}_" in content and "postgresql_using='gin'" in content
    )

    if jsonb_found >= 5:
        validation_results["implements_jsonb_gin_indexes"] = True
        validation_results["summary"].append(
            f"[OK] JSONB GIN indexes implemented ({jsonb_found}/7)"
        )
    ...
    error_handling = ["try:", "except Exception:", "print", "already exists"]
...
def validate_erd_compliance() -> Dict[str, Any]:
    ...
    if session_file.exists():
        ...
        unique_checks.append(
            "unique=True" in session_content and "refresh_token_hash" in session_content
        )
    ...
    if invoice_file.exists():
        ...
        financial_checks.append(
            "amount_cents" in invoice_content and "BigInteger" in invoice_content
        )
    ...
    if payment_file.exists():
        ...
        financial_checks.append(
            "amount_cents" in payment_content and "BigInteger" in payment_content
        )
    ...
    if invoice_file.exists():
        ...
        turkish_features.append(
            "KDV" in invoice_content or "tax_rate_percent: float = 20.0" in invoice_content
        )  # Turkish VAT

    if sum(turkish_features) >= 3:
        compliance_results["turkish_compliance_addressed"] = True
        compliance_results["summary"].append(
            "[OK] Turkish financial compliance (GDPR/KVKV + KDV) implemented"
        )
...
def validate_performance_optimization() -> Dict[str, Any]:
    ...
    if migration_file.exists():
        ...
        composite_found = sum(
            1 for pattern in composite_patterns if re.search(pattern, content, re.DOTALL)
        )
        ...
        if "postgresql_using='gin'" in content and "gin_" in content:
            ...
            perf_results["summary"].append(
                "[OK] JSONB GIN indexes with PostgreSQL 17.6 optimizations"
            )
...
def main():
    ...
    migration_score = sum(
        1 for key, value in migration_results.items() if key != "summary" and value is True
    )
    ...
    erd_score = sum(1 for key, value in erd_results.items() if key != "summary" and value is True)
    ...
    perf_score = sum(1 for key, value in perf_results.items() if key != "summary" and value is True)
