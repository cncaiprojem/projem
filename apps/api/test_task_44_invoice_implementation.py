...
def test_invoice_vat_calculation():
    ...
    assert amounts_dict['amount'] == expected_amount, f"Expected amount {expected_amount}, got {amounts_dict['amount']}"
    assert amounts_dict['vat'] == expected_vat, f"Expected VAT {expected_vat}, got {amounts_dict['vat']}"
    assert amounts_dict['total'] == expected_total, f"Expected total {expected_total}, got {amounts_dict['total']}"
...
def test_invoice_model_fields():
    ...
    required_fields = {
        'id', 'user_id', 'license_id', 'number', 'amount', 'currency', 
        'vat', 'total', 'paid_status', 'issued_at', 'pdf_url', 
    ...
    assert actual_statuses == expected_statuses, f"Expected statuses {expected_statuses}, got {actual_statuses}"
...
def test_invoice_service_basic():
    ...
    print(f"Service calculation - Amount: {amounts['amount']}, VAT: {amounts['vat']}, Total: {amounts['total']}")
