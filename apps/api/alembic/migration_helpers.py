...
def create_enum_type_safe(enum_name: str, values: Sequence[str], schema: str | None = None) -> None:
    ...
    if not enum_name.replace("_", "").replace("-", "").isalnum():
        raise ValueError(
            f"Invalid enum name: {enum_name}. Only alphanumeric characters, underscores, and hyphens are allowed."
        )

    if schema and not schema.replace("_", "").replace("-", "").isalnum():
        raise ValueError(
            f"Invalid schema name: {schema}. Only alphanumeric characters, underscores, and hyphens are allowed."
        )

    for value in values:
        if "'" in value or '"' in value or ";" in value or "--" in value:
            raise ValueError(
                f"Invalid enum value: {value}. Values cannot contain quotes, semicolons, or SQL comments."
            )
...
def create_enum_type(enum_name: str, values: Sequence[str], schema: str | None = None) -> None:
    ...
    try:
        ...
        result = (
            op.get_bind()
            .execute(check_query, {"enum_name": enum_name, "schema_name": schema or "public"})
            .fetchone()
        )
...
def create_gin_index(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        op.create_index(index_name, table_name, [column_name], **index_options)
...
def add_check_constraint(
    table_name: str, constraint_name: str, condition: str, schema: str | None = None
) -> None:
    """
    ...
    try:
        ...
        op.create_check_constraint(full_constraint_name, table_name, condition, schema=schema)
...
def create_trigger(
    ...
    if not trigger_name.replace("_", "").replace("-", "").isalnum():
        raise ValueError(
            f"Invalid trigger name: {trigger_name}. Only alphanumeric characters, underscores, and hyphens are allowed."
        )
    ...
    if not function_name.replace("_", "").replace("-", "").isalnum():
        raise ValueError(
            f"Invalid function name: {function_name}. Only alphanumeric characters, underscores, and hyphens are allowed."
        )
...
def drop_trigger(
    trigger_name: str, table_name: str, schema: str | None = None, if_exists: bool = True
) -> None:
    """
    ...
    try:
        ...
        trigger_sql = text(
            f"DROP TRIGGER {if_exists_clause}{trigger_name} ON {schema_prefix}{table_name}"
        )
...
def create_materialized_view(
    view_name: str, query: str, schema: str | None = None, with_data: bool = True
) -> None:
    """
...
def refresh_materialized_view(
    view_name: str, schema: str | None = None, concurrently: bool = False
) -> None:
    """
...
def add_column_comment(
    table_name: str, column_name: str, comment: str, schema: str | None = None
) -> None:
    """
    ...
    if not column_name.replace("_", "").replace("-", "").isalnum():
        raise ValueError(
            f"Invalid column name: {column_name}. Only alphanumeric characters, underscores, and hyphens are allowed."
        )
    ...
    try:
        ...
        comment_sql = text(
            f"COMMENT ON COLUMN {schema_prefix}{table_name}.{column_name} IS '{escaped_comment}'"
        )
...
def validate_migration_safety(table_name: str, schema: str | None = None) -> bool:
    ...
    try:
        ...
        result = (
            op.get_bind()
            .execute(size_query, {"table_name": table_name, "schema_name": schema})
            .scalar()
        )
    ...
    try:
        ...
        pending_locks = op.get_bind().execute(lock_query, {"table_name": table_name}).scalar()
