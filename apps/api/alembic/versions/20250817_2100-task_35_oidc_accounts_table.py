...
revision = "20250817_2100"
down_revision = "20250817_2045"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "id", postgresql.UUID(as_uuid=False), primary_key=True, comment="OIDC account UUID"
        ),
        # Foreign key to users table
        sa.Column(
            "user_id", sa.Integer(), nullable=False, comment="Reference to local user account"
        ),
        # OIDC provider information
        sa.Column(
            "provider", sa.String(50), nullable=False, comment="OIDC provider name (e.g., google)"
        ),
        # OIDC subject identifier (unique per provider)
        sa.Column(
            "sub", sa.String(255), nullable=False, comment="OIDC subject identifier from provider"
        ),
        # User profile information from OIDC provider
        sa.Column(
            "email", sa.String(255), nullable=False, comment="Email address from OIDC provider"
        ),
        sa.Column(
            "email_verified",
            sa.Boolean(),
    ...
    op.create_index(
        "ix_oidc_accounts_provider_sub", "oidc_accounts", ["provider", "sub"], unique=True
    )
    ...
    op.create_index("ix_oidc_accounts_email", "oidc_accounts", ["email"])

    # Index for provider lookups
    op.create_index("ix_oidc_accounts_provider", "oidc_accounts", ["provider"])

    # Index for active account queries
    op.create_index("ix_oidc_accounts_is_active", "oidc_accounts", ["is_active"])

    # Performance index for audit queries
    op.create_index("ix_oidc_accounts_last_login", "oidc_accounts", ["last_login_at"])

    # Composite index for active accounts by provider
    op.create_index("ix_oidc_accounts_provider_active", "oidc_accounts", ["provider", "is_active"])

    # Add check constraints for data integrity

    # Ensure login_count is non-negative
    op.create_check_constraint(
        "ck_oidc_accounts_login_count_non_negative", "oidc_accounts", "login_count >= 0"
    )

    # Ensure provider is not empty
    op.create_check_constraint(
        "ck_oidc_accounts_provider_not_empty", "oidc_accounts", "provider != ''"
    )

    # Ensure sub is not empty
    op.create_check_constraint("ck_oidc_accounts_sub_not_empty", "oidc_accounts", "sub != ''")

    # Ensure email is not empty
    op.create_check_constraint("ck_oidc_accounts_email_not_empty", "oidc_accounts", "email != ''")
