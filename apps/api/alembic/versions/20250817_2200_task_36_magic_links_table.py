...
revision = '20250817_2200_task_36'
down_revision = '20250817_2100_task_35'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table('magic_links',
        ...
        sa.Column('consumed_ip_address', postgresql.INET(), nullable=True,
        ...
        sa.Column('consumed_device_fingerprint', sa.String(length=512), nullable=True,
        ...
        sa.Column('consumption_attempts', sa.Integer(), nullable=False, default=0,
        ...
        sa.CheckConstraint('consumption_attempts >= 0', 
                          name='ck_magic_links_consumption_attempts_non_negative'),
        sa.CheckConstraint('consumed_at IS NULL OR consumed_at >= issued_at',
    ...
    op.create_index('idx_magic_links_email_issued', 'magic_links', 
                    ['email', 'issued_at'])
    op.create_index('idx_magic_links_nonce_unique', 'magic_links', 
                    ['nonce'], unique=True)
    op.create_index('idx_magic_links_issued_at', 'magic_links', 
                    ['issued_at'])
    
    # Consumption tracking indexes
    op.create_index('idx_magic_links_consumed_at', 'magic_links', 
                    ['consumed_at'],
                    postgresql_where=sa.text('consumed_at IS NOT NULL'))
    
    # Security monitoring indexes
    op.create_index('idx_magic_links_ip_address', 'magic_links', 
                    ['ip_address'],
                    postgresql_where=sa.text('ip_address IS NOT NULL'))
    
    # Active links index for rate limiting
    op.create_index('idx_magic_links_active', 'magic_links', 
                    ['email', 'issued_at'],
                    postgresql_where=sa.text('consumed_at IS NULL AND invalidated_at IS NULL'))
    
    # Cleanup index for expired links
    op.create_index('idx_magic_links_expired', 'magic_links', 
                    ['issued_at'],
                    postgresql_where=sa.text('consumed_at IS NULL AND invalidated_at IS NULL'))
