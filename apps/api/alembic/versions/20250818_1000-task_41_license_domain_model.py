...
revision: str = "task_41_license_domain"
down_revision: Union[str, None] = "20250818_0000-task_37_mfa_totp_tables"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("user_id", sa.BigInteger(), nullable=False, comment="User who owns this license"),
        sa.Column(
            "type",
            sa.String(length=10),
            nullable=False,
        ...
        sa.Column(
            "reason", sa.Text(), nullable=True, comment="Reason for cancellation (if canceled)"
        ),
        sa.Column(
            "starts_at",
        ...
        sa.CheckConstraint("type IN ('3m', '6m', '12m')", name="ck_licenses_valid_type"),
        sa.CheckConstraint(
            "status IN ('active', 'expired', 'canceled')", name="ck_licenses_valid_status"
        ),
        sa.CheckConstraint("ends_at > starts_at", name="ck_licenses_valid_period"),
        sa.CheckConstraint(
            "(status != 'canceled' OR canceled_at IS NOT NULL)",
            name="ck_licenses_canceled_has_timestamp",
        ),
        sa.CheckConstraint(
            "(status != 'canceled' OR reason IS NOT NULL)", name="ck_licenses_canceled_has_reason"
        ),
        sa.CheckConstraint(
    ...
    op.create_index("idx_licenses_scope", "licenses", ["scope"], postgresql_using="gin")
    ...
    op.create_table(
        ...
        sa.Column("license_id", sa.BigInteger(), nullable=False, comment="License being audited"),
        sa.Column(
            "user_id",
            sa.BigInteger(),
            nullable=True,
        ...
        sa.Column(
            "reason", sa.Text(), nullable=True, comment="Human-readable reason for the action"
        ),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
    ...
    op.create_index(
        "idx_license_audit_license_event", "license_audit", ["license_id", "event_type"]
    )
    ...
    op.execute(
        "COMMENT ON TABLE licenses IS 'Task 4.1: Ultra-enterprise license domain with state transitions'"
    )
    op.execute(
        "COMMENT ON TABLE license_audit IS 'Task 4.1: License audit trail with hash-chain integrity'"
    )
