...
revision = "20250817_2030_task_31_enterprise_auth_fields"
down_revision = "20250817_1530_init_basic_tables"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.add_column(
        "users",
        sa.Column(
            "password_salt", sa.String(64), nullable=True, comment="Per-user salt for Argon2"
        ),
    )
    op.add_column(
        "users",
    ...
    op.add_column(
        "users",
        sa.Column(
            "last_login_user_agent", sa.Text, nullable=True, comment="User agent of last login"
        ),
    )
    op.add_column(
        "users",
    ...
    op.add_column(
        "users",
        sa.Column("auth_metadata", JSONB, nullable=True, comment="Authentication-related metadata"),
    )
    ...
    op.create_index(
        "idx_users_full_name", "users", ["full_name"], postgresql_where="full_name IS NOT NULL"
    )
    op.create_index("idx_users_data_processing_consent", "users", ["data_processing_consent"])

    # Add check constraints for data integrity
    op.create_check_constraint(
        "ck_users_failed_login_attempts_non_negative", "users", "failed_login_attempts >= 0"
    )
    op.create_check_constraint(
        "ck_users_total_login_count_non_negative", "users", "total_login_count >= 0"
    )
    op.create_check_constraint(
        "ck_users_password_reset_attempts_non_negative", "users", "password_reset_attempts >= 0"
    )
    op.create_check_constraint(
