...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "code_hash", sa.String(64), nullable=False, comment="SHA-256 hash of backup code"
        ),
        sa.Column(
            "code_hint",
            sa.String(8),
        ...
        sa.Column(
            "used_user_agent", sa.Text(), nullable=True, comment="User agent when code was used"
        ),
        sa.Column(
            "expires_at",
            sa.DateTime(timezone=True),
    ...
    op.create_check_constraint(
        "ck_mfa_backup_codes_expires_after_creation", "mfa_backup_codes", "expires_at > created_at"
    )
    op.create_check_constraint(
    ...
    op.create_check_constraint(
        "ck_users_mfa_backup_codes_count_non_negative", "users", "mfa_backup_codes_count >= 0"
    )
...
def downgrade():
    ...
    op.drop_constraint(
        "ck_mfa_backup_codes_expires_after_creation", "mfa_backup_codes", type_="check"
    )
