...
revision = '20250818_1100_task_44'
down_revision = '20250818_1000_task_41'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table('invoices',
        ...
        sa.Column('license_id', sa.BigInteger(), nullable=False, comment='License this invoice is for'),
        
        # Invoice identification - Task 4.4 numbering scheme
        sa.Column('number', sa.String(length=20), nullable=False, comment='Unique invoice number: YYYYMM-SEQ-CNCAI format'),
        
        # Financial amounts - Task 4.4 specification with NUMERIC(12,2) precision
        sa.Column('amount', sa.NUMERIC(precision=12, scale=2), nullable=False, comment='Invoice base amount before VAT'),
        sa.Column('currency', sa.CHAR(length=3), server_default=sa.text("'TRY'"), nullable=False, comment='Invoice currency - fixed to TRY'),
        sa.Column('vat', sa.NUMERIC(precision=12, scale=2), nullable=False, comment='VAT amount at 20% Turkish KDV rate'),
        sa.Column('total', sa.NUMERIC(precision=12, scale=2), nullable=False, comment='Total amount including VAT'),
        
        # Payment status - Task 4.4 specification
        sa.Column('paid_status', paid_status_enum, server_default=sa.text("'unpaid'"), nullable=False, comment='Payment status: unpaid, pending, paid, failed, refunded'),
        
        # Issued timestamp - Task 4.4: Used in numbering scheme
        sa.Column('issued_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False, comment='When invoice was issued (UTC)'),
        ...
        sa.Column('provider_payment_id', sa.Text(), nullable=True, comment='External payment provider transaction ID'),
        ...
        sa.CheckConstraint("number ~ '^[0-9]{6}-[0-9]{6}-CNCAI$'", name='ck_invoices_number_format'),
    )
