                ...
                for dlq_name in dlq_queues:
                    try:
                        ...
                        if message_count <= max_messages:
                            logger.info(
                                f"DLQ {dlq_name} message count ({message_count}) within limit ({max_messages})"
                            )
                            continue
                        ...
                        for _ in range(messages_to_process):
                            ...
                            if message_age_hours > 168:  # 7 gün
                                ...
                                logger.debug(
                                    f"Discarded old DLQ message from {dlq_name} (age: {message_age_hours:.1f}h)"
                                )
                            else:
                                ...
                                logger.debug(
                                    f"Requeued DLQ message from {dlq_name} to {original_queue}"
                                )
