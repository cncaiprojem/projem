...
class PIIMaskingService:
    ...
    def __init__(self, salt: str | None = None):
        ...
        self.turkish_phone_pattern = re.compile(r'(\+90|0)?\s*(\d{3})\s*(\d{3})\s*(\d{2})\s*(\d{2})')
        self.iban_pattern = re.compile(r'\bTR\d{2}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{2}\b')
    ...
    def mask_email(self, email: str, level: MaskingLevel = MaskingLevel.MEDIUM) -> str:
        ...
        try:
            ...
            if level == MaskingLevel.LIGHT:
                ...
                masked_domain = domain_parts[0][0] + '*' * (len(domain_parts[0]) - 1) if len(domain_parts[0]) > 1 else '*'
                ...
                domain_suffix = '.' + '.'.join(
                    self._mask_preserving_boundaries(part) for part in domain_parts[1:]
                ) if len(domain_parts) > 1 else ''
    ...
    def mask_iban(self, iban: str, level: MaskingLevel = MaskingLevel.HEAVY) -> str:
        ...
        if level == MaskingLevel.FULL or level == MaskingLevel.HEAVY:
            ...
            formatted = f"TR{clean_iban[2:4]} {clean_iban[4:8]} {clean_iban[8:12]} **** **** **** **"
            return formatted
    ...
    def create_masked_metadata(
        ...
        for key, value in original_data.items():
            if key in preserve_keys:
                ...
                masked_data[key] = [
                    self.create_masked_metadata(item, classification, preserve_keys) if isinstance(item, dict)
                    else self.mask_text_content(str(item), classification) if isinstance(item, str)
                    else item
                    for item in value
                ]
            else:
