...
class TokenService:
    ...
    def create_refresh_session(
        ...
        try:
            ...
            logger.error("Refresh session creation failed", exc_info=True, extra={
                'operation': 'create_refresh_session',
                'user_id': user.id,
    ...
    def rotate_refresh_token(
        ...
        try:
            ...
            if chain_length >= self.max_rotation_chain:
                logger.warning("Excessive rotation chain detected", extra={
            ...
            if device_fingerprint and current_session.device_fingerprint:
                if device_fingerprint != current_session.device_fingerprint:
                    self._log_security_event(
                        db, current_session.user_id, 'DEVICE_FINGERPRINT_MISMATCH',
            ...
            logger.error("Refresh token rotation failed", exc_info=True, extra={
                'operation': 'rotate_refresh_token',
                'error_type': type(e).__name__
            })
            raise TokenServiceError(
