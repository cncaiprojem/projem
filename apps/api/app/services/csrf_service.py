...
class CSRFService:
    ...
    **Integration:**
    ...
    def __init__(self):
        ...
        self.browser_user_agents = ["Mozilla", "Chrome", "Safari", "Firefox", "Edge", "Opera"]
    ...
    def generate_csrf_token(
        ...
        **Security Implementation:**
        ...
        try:
            ...
            signature = hmac.new(self.hmac_key.encode(), token_payload, hashlib.sha256).digest()
            ...
            self._log_csrf_event(
                ...
                user_agent,
                {
                    ...
                    "expires_at": (
                        datetime.now(timezone.utc) + timedelta(seconds=self.token_lifetime_seconds)
                    ).isoformat(),
            ...
            raise CSRFError("ERR-CSRF-GENERATION-FAILED", "CSRF token oluşturma başarısız")

    def set_csrf_cookie(
        self, response: Response, csrf_token: str, secure: Optional[bool] = None
    ) -> None:
        """
        ...
        **Cookie Security Configuration:**
        ...
        try:
            ...
            raise CSRFError("ERR-CSRF-COOKIE-SET-FAILED", "CSRF cookie ayarlama başarısız")
    ...
    def validate_csrf_token(
        ...
        **Validation Logic:**
        ...
        try:
            ...
            if not self._is_csrf_protection_required(request, require_auth):
                logger.debug(
                    ...
                    extra={
                        ...
                        "user_agent": client_info["user_agent"][:50]
            ...
            if not self._is_valid_token_format(cookie_token) or not self._is_valid_token_format(
                header_token
            ):
                self._log_csrf_event(
                    db,
    ...
    def _is_token_expired(self, token: str) -> bool:
        ...
        try:
            ...
            expected_signature = hmac.new(
                self.hmac_key.encode(), token_payload, hashlib.sha256
            ).digest()
    ...
    def _check_token_generation_rate_limit(self, ip_address: str) -> None:
        ...
        self._rate_limit_cache[ip_address] = [
            req_time for req_time in requests if req_time > minute_ago
        ]

        if len(self._rate_limit_cache[ip_address]) >= self.max_tokens_per_minute:
            raise CSRFError(
                "ERR-CSRF-RATE-LIMIT", "CSRF token istekleri çok sık. Bir dakika bekleyin."
            )
    ...
    def _log_csrf_event(
        ...
        try:
            ...
            logger.error(
                "Failed to log CSRF security event",
                exc_info=True,
                extra={
