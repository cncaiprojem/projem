...
class S3Service:
    ...
    def download_file(self, bucket: str, object_key: str, local_path: Union[str, Path]) -> Path:
        """
        ...
        try:
            ...
            self.client.fget_object(
                bucket_name=bucket, object_name=object_key, file_path=str(local_path)
            )
    ...
    def delete_object(self, bucket: str, object_key: str) -> bool:
        ...
        try:
            ...
            logger.info("Object deleted successfully", bucket=bucket, object_key=object_key)

            return True

        except S3Error as e:
            logger.error(
                "Failed to delete object", bucket=bucket, object_key=object_key, error=str(e)
            )
            raise

    def list_objects(
        self, bucket: str, prefix: Optional[str] = None, recursive: bool = True
    ) -> list:
        """
        ...
        try:
            ...
            logger.info("Listed objects", bucket=bucket, prefix=prefix, count=len(objects))
    ...
    def get_object_info(self, bucket: str, object_key: str) -> Dict:
        ...
        try:
            ...
            logger.info(
                "Retrieved object info", bucket=bucket, object_key=object_key, size=stat.size
            )
            ...
            logger.error(
                "Failed to get object info", bucket=bucket, object_key=object_key, error=str(e)
            )
            raise
    ...
    def copy_object(
        ...
        try:
            ...
            self.client.copy_object(
                bucket_name=dest_bucket, object_name=dest_key, source=copy_source, metadata=metadata
            )
    ...
    def create_temp_download_url(
        self, bucket: str, object_key: str, filename: Optional[str] = None, expiry_minutes: int = 15
    ) -> str:
        """
    ...
    async def set_object_legal_hold(
        self, bucket_name: str, object_key: str, legal_hold: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                "Legal hold requested but may not be supported",
                bucket=bucket_name,
                object_key=object_key,
            ...
            logger.warning(
                "Legal hold not supported or failed",
                bucket=bucket_name,
                object_key=object_key,
                error=str(e),
            )
