...
class OIDCService:
    ...
    async def get_google_config(self) -> Dict[str, Any]:
        ...
        # Check cache
        if (
            self._google_config_cache
            and self._google_config_expires
            and now < self._google_config_expires
        ):
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                logger.debug(
                    "Google OIDC configuration retrieved",
                    extra={
                        "operation": "get_google_config",
                        "issuer": config.get("issuer"),
                        "endpoints_count": len(
                            [k for k in config.keys() if k.endswith("_endpoint")]
                        ),
                    },
                )
            ...
            logger.error(
                "Failed to retrieve Google OIDC configuration",
                exc_info=True,
                extra={
                    "operation": "get_google_config",
            ...
            raise OIDCServiceError("ERR-OIDC-CONFIG-FAILED", "OIDC yapılandırması alınamadı")
    ...
    def generate_pkce_pair(self) -> Tuple[str, str]:
        ...
        code_verifier = (
            base64.urlsafe_b64encode(secrets.token_bytes(96)).decode("utf-8").rstrip("=")
        )
    ...
    async def store_oauth_state(
        self, redis_client, state: str, pkce_verifier: str, nonce: str, redirect_uri: str
    ) -> None:
        """
        ...
        await redis_client.setex(
            f"{self.state_prefix}{state}", self.state_expire_seconds, json.dumps(state_data)
        )

        # Also store PKCE verifier separately for additional security
        await redis_client.setex(
            f"{self.pkce_prefix}{state}", self.pkce_verifier_expire_seconds, pkce_verifier
        )

        # Store nonce for validation
        await redis_client.setex(f"{self.nonce_prefix}{state}", self.state_expire_seconds, nonce)

        logger.debug(
            "OAuth state stored securely",
            extra={
    ...
    async def retrieve_and_validate_state(
        ...
        try:
            ...
            if not state_json:
                raise OIDCServiceError("ERR-OIDC-STATE", "OIDC state doğrulaması başarısız")
            ...
            if not nonce:
                raise OIDCServiceError("ERR-OIDC-NONCE", "OIDC nonce doğrulaması başarısız")
            ...
            logger.debug(
                "OAuth state validated and cleaned",
                extra={
            ...
            raise OIDCServiceError("ERR-OIDC-STATE-CORRUPT", "OIDC state verisi bozuk")
        except Exception as e:
            logger.error(
                "OAuth state validation failed",
                exc_info=True,
                extra={"operation": "retrieve_and_validate_state", "error_type": type(e).__name__},
            )
            raise OIDCServiceError(
    ...
    async def create_authorization_url(
        ...
        try:
            ...
            logger.error(
                ...
                exc_info=True,
                extra={"operation": "create_authorization_url", "error_type": type(e).__name__},
            )
            raise OIDCServiceError(
    ...
    async def exchange_code_for_tokens(
        ...
        try:
            ...
            if not id_token:
                raise OIDCServiceError("ERR-OIDC-NO-ID-TOKEN", "OIDC ID token bulunamadı")
            ...
            logger.error(
                "Token exchange failed",
                exc_info=True,
                extra={"operation": "exchange_code_for_tokens", "error_type": type(e).__name__},
            )
            raise OIDCServiceError(
    ...
    async def _get_jwks_client(self) -> PyJWKClient:
        ...
        if self._jwks_client and self._jwks_client_expires and now < self._jwks_client_expires:
        ...
        try:
            ...
            if not jwks_uri:
                raise OIDCServiceError("ERR-OIDC-NO-JWKS-URI", "OIDC JWKS endpoint bulunamadı")
            ...
            logger.debug(
                "JWKS client created",
                extra={"operation": "_get_jwks_client", "jwks_uri": jwks_uri, "cache_ttl": 3600},
            )
            ...
            logger.error(
                ...
                exc_info=True,
                extra={"operation": "_get_jwks_client", "error_type": type(e).__name__},
            )
            raise OIDCServiceError(
                "ERR-OIDC-JWKS-CLIENT-FAILED", "OIDC JWKS istemcisi oluşturulamadı"
            )
    ...
    async def _verify_id_token(self, id_token: str, expected_nonce: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if claims.get("iss") != "https://accounts.google.com":
                raise OIDCServiceError("ERR-OIDC-INVALID-ISSUER", "OIDC issuer geçersiz")

            if claims.get("aud") != self.google_client_id:
                raise OIDCServiceError("ERR-OIDC-INVALID-AUDIENCE", "OIDC audience geçersiz")
            ...
            if now - iat > 86400:  # 24 hours
                raise OIDCServiceError("ERR-OIDC-TOKEN-TOO-OLD", "OIDC token çok eski")

            logger.info(
                "ID token signature and claims verified successfully",
                extra={
            ...
            logger.error(
                "ID token JWT verification failed",
                exc_info=True,
                extra={
                    "operation": "_verify_id_token",
                    "error_type": type(e).__name__,
            ...
            raise OIDCServiceError(
                "ERR-OIDC-TOKEN-INVALID", "OIDC token imza doğrulaması başarısız"
            )
        except Exception as e:
            logger.error(
                "ID token verification failed with unexpected error",
                exc_info=True,
                extra={"operation": "_verify_id_token", "error_type": type(e).__name__},
            )
            raise OIDCServiceError(
                "ERR-OIDC-TOKEN-VERIFICATION-FAILED", "OIDC token doğrulaması başarısız"
            )
    ...
    async def authenticate_or_link_user(
        ...
        try:
            ...
            raise OIDCServiceError("ERR-OIDC-AUTH-FAILED", "OIDC kimlik doğrulama başarısız")
    ...
    def _log_oidc_audit_event(
        ...
        try:
            ...
            audit_log = AuditLog(
                user_id=user_id, action=action, description=description, details=masked_details
            )
            db.add(audit_log)
            db.flush()
        except Exception as e:
            logger.error(
                "Failed to log OIDC audit event",
                exc_info=True,
    ...
    def _log_oidc_security_event(
        ...
        try:
            ...
            logger.error(
                "Failed to log OIDC security event",
                exc_info=True,
