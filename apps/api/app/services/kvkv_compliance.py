...
class KVKVComplianceService:
    ...
    def record_consent(
        ...
        try:
            ...
            if not user:
                logger.error("User not found for consent recording", extra={"user_id": user_id})
                return False
            ...
            self._create_consent_audit_log(
                db, user_id, data_processing_consent, marketing_consent, ip_address, user_agent
            )

            db.commit()

            logger.info(
                "KVKV consent recorded",
                extra={
                    "user_id": user_id,
    ...
    def withdraw_consent(
        ...
        try:
            ...
            if consent_type == "data_processing":
                ...
                logger.warning(
                    "Data processing consent withdrawn - account deactivated",
                ...
                logger.info("Marketing consent withdrawn", extra={"user_id": user_id})
        ...
        try:
            ...
            data_summary = {
                ...
                "consent_information": {
                    ...
                    "data_processing_consent_at": user.data_processing_consent_at.isoformat()
                    if user.data_processing_consent_at
                    else None,
                    "marketing_consent": user.marketing_consent,
                    "marketing_consent_at": user.marketing_consent_at.isoformat()
                    if user.marketing_consent_at
                    else None,
                    "consent_version": user.auth_metadata.get("kvkv_consent_version")
                    if user.auth_metadata
                    else None,
                },
                "account_information": {
                    ...
                    "last_successful_login_at": user.last_successful_login_at.isoformat()
                    if user.last_successful_login_at
                    else None,
                ...
                "security_information": {
                    ...
                    "account_locked_until": user.account_locked_until.isoformat()
                    ...
                    "last_failed_login_at": user.last_failed_login_at.isoformat()
            ...
            data_summary["related_data_counts"] = {
                ...
                "security_events": db.query(SecurityEvent)
            ...
            logger.info(
                "User data summary generated for KVKV compliance",
                extra={
                    "user_id": user_id,
            ...
            logger.error(
                "Failed to generate user data summary",
                exc_info=True,
                extra={"user_id": user_id, "error_type": type(e).__name__},
            )
            return {}
    ...
    def request_data_deletion(
        ...
        try:
            ...
            if not self._can_request_deletion(user):
                logger.warning(
                    "Data deletion request denied - active obligations",
                    extra={"user_id": user_id, "account_status": user.account_status},
                )
                return False
            ...
            user.auth_metadata.update(
                {
                    ...
                    ).isoformat(),  # 30-day grace period
                }
            )

            # Create audit log entry
            self._create_deletion_request_audit_log(db, user_id, ip_address, user_agent)
    ...
    def export_user_data(self, db: Session, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "User data exported for KVKV compliance",
                extra={
                    "user_id": user_id,
    ...
    def _create_deletion_request_audit_log(
        self, db: Session, user_id: int, ip_address: Optional[str], user_agent: Optional[str]
    ) -> None:
    ...
    def _get_processing_purposes(self, user: User) -> List[str]:
        ...
        if user.data_processing_consent:
            purposes.extend(["authentication", "security", "service_provision", "legal_compliance"])
