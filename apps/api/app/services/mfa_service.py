...
class TOTPService:
    ...
    def _decrypt_secret(self, encrypted_secret: str) -> str:
        ...
        try:
            ...
            raise MFAError(
                "ERR-MFA-DECRYPTION-FAILED", "MFA şifresi çözülürken hata oluştu", {"error": str(e)}
            )
    ...
    def _check_rate_limit(self, user_id: int) -> bool:
        ...
        self._verify_attempts[user_id] = [
            timestamp
            for timestamp in self._verify_attempts[user_id]
    ...
    def _generate_backup_codes(self) -> List[str]:
        ...
        for _ in range(self.BACKUP_CODE_COUNT):
            ...
            code = "".join(
                secrets.choice("23456789ABCDEFGHJKLMNPQRSTUVWXYZ")
    ...
    def setup_mfa(
        ...
        """
        if user.mfa_enabled:
            raise MFAError("ERR-MFA-ALREADY-ENABLED", "MFA zaten aktif")
        ...
        logger.info(
            "MFA setup initiated",
            extra={"user_id": user.id, "email": user.email, "ip_address": ip_address},
        )
    ...
    def verify_and_enable_mfa(
        ...
        if not user.mfa_secret_encrypted:
            raise MFAError("ERR-MFA-SETUP-NOT-STARTED", "MFA kurulumu başlatılmamış")
        ...
        if not self._check_rate_limit(user.id):
            ...
            raise MFAError("ERR-MFA-RATE-LIMITED", "Çok fazla doğrulama denemesi. Lütfen bekleyin.")
        ...
        try:
            ...
            raise MFAError(
                "ERR-MFA-SYSTEM-ERROR", "MFA doğrulama sırasında sistem hatası", {"error": str(e)}
            )
    ...
    def verify_totp_code(
        ...
        try:
            ...
            logger.error(
                "Unexpected error during TOTP verification",
                exc_info=True,
                extra={"user_id": user.id, "email": user.email},
            )
            return False
    ...
    def disable_mfa(
        ...
        return {"message": "MFA başarıyla devre dışı bırakıldı"}
