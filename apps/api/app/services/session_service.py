...
class SessionService:
    ...
    def create_session(
        ...
        try:
            ...
            if not user:
                raise SessionSecurityError("ERR-SESSION-USER-NOT-FOUND", "Kullanıcı bulunamadı")

            if not user.can_attempt_login():
                raise SessionSecurityError(
                    "ERR-SESSION-USER-INACTIVE", "Kullanıcı hesabı aktif değil"
                )
            ...
            if len(active_sessions) >= self.max_sessions_per_user:
                ...
                self.revoke_session(
                    db, oldest_session.id, "max_sessions_exceeded", ip_address, user_agent
                )
            ...
            raise SessionSecurityError(
                "ERR-SESSION-CREATION-FAILED", "Oturum oluşturma başarısız, lütfen tekrar deneyin"
            )
    ...
    def rotate_session(
        ...
        try:
            ...
            current_session = (
                db.query(Session)
                .filter(
                    Session.refresh_token_hash == refresh_token_hash, Session.revoked_at.is_(None)
                )
                .first()
            )
            ...
            if not current_session or not current_session.is_active:
                ...
                if revoked_session:
                    ...
                    self._handle_refresh_token_reuse(
                        db, revoked_session.user_id, ip_address, user_agent, revoked_session.id
                    )

                raise SessionSecurityError(
                    "ERR-SESSION-INVALID-TOKEN", "Geçersiz veya süresi dolmuş oturum"
                )
            ...
            if current_session.is_expired:
                ...
                raise SessionSecurityError(
                    "ERR-SESSION-EXPIRED", "Oturum süresi dolmuş, lütfen tekrar giriş yapın"
                )
            ...
            if chain_length >= self.max_rotation_chain_length:
                ...
                self.revoke_session(
                    db, current_session.id, "rotation_limit_exceeded", ip_address, user_agent
                )
            ...
            if (
                ...
                and device_fingerprint != current_session.device_fingerprint
            ):
                self._log_security_event(
                    db,
                    current_session.user_id,
            ...
            self._create_audit_log(
                ...
                {"old_session_id": str(current_session.id), "new_session_id": str(new_session.id)},
            )
    ...
    def revoke_all_user_sessions(
        ...
        try:
            ...
            if revoked_count > 0:
                self._log_security_event(
                    ...
                    user_agent,
                    {
                        ...
                        "reason": reason,
                        "preserved_session_id": str(except_session_id)
                        if except_session_id
                        else None,
                    },
                )
            ...
            db.rollback()
            logger.error(
                "Bulk session revocation failed",
                exc_info=True,
                extra={
    ...
    def cleanup_expired_sessions(self, db: DBSession) -> int:
        ...
        try:
            ...
            expired_sessions = (
                db.query(Session)
                .filter(
                    Session.expires_at < datetime.now(timezone.utc), Session.revoked_at.is_(None)
                )
                .all()
            )
    ...
    def get_session_by_token(self, db: DBSession, refresh_token: str) -> Optional[Session]:
        """
    ...
    def _hash_refresh_token(self, token: str) -> str:
        ...
        return hmac.new(self.session_secret.encode(), token.encode(), hashlib.sha512).hexdigest()
    ...
    def _analyze_device_fingerprint(
        self, db: DBSession, user_id: int, device_fingerprint: str, ip_address: Optional[str]
        ...
        try:
            ...
            recent_fingerprints = (
                db.query(Session.device_fingerprint)
                .filter(
                    Session.user_id == user_id,
            ...
            logger.error(
                "Device fingerprint analysis failed",
                exc_info=True,
                extra={
    ...
    def _handle_refresh_token_reuse(
        ...
        revoked_count = self.revoke_all_user_sessions(
            db, user_id, "token_reuse_detected", ip_address=ip_address, user_agent=user_agent
        )
    ...
    def _log_security_event(
        ...
        try:
            ...
            logger.error(
                "Failed to log session security event",
                exc_info=True,
                extra={
                    "event_type": event_type,
                    "user_id": user_id,
    ...
    def _create_audit_log(
        ...
        try:
            ...
            logger.error(
                "Failed to create session audit log",
                exc_info=True,
                extra={
                    "action": action,
                    "user_id": user_id,
