...
class LicenseService:
    ...
    def _create_audit_log(
        ...
        previous_audit = (
            ...
            .filter(LicenseAudit.license_id == license.id)
            .order_by(LicenseAudit.id.desc())
            .first()
        )
    ...
    def assign_license(
        ...
        audit = LicenseService._create_audit_log(
            ...
            delta={"action": "assign", "duration_months": months},
            user_id=user_id,
            actor_type=actor_type,
            actor_id=actor_id,
            ip_address=ip_address,
    ...
    def extend_license(
        ...
        old_state = {"ends_at": license.ends_at.isoformat(), "type": license.type}
        ...
        audit = LicenseService._create_audit_log(
            ...
            old_state=old_state,
            new_state={"ends_at": license.ends_at.isoformat(), "type": license.type},
            delta={
    ...
    def cancel_license(
        ...
        # Store old state for audit
        old_state = {"status": license.status, "canceled_at": None, "reason": None}
        ...
        audit = LicenseService._create_audit_log(
            ...
            old_state=old_state,
            new_state={"status": "canceled", "canceled_at": now.isoformat(), "reason": reason},
            delta={
    ...
    @staticmethod
    def expire_licenses(db: Session, batch_size: int = 100) -> int:
        ...
        expired_licenses = (
            db.query(License)
            .filter(and_(License.status == "active", License.ends_at <= now))
            .limit(batch_size)
            .all()
        )
        ...
        for license in expired_licenses:
            # Store old state for audit
            old_state = {"status": license.status}
            ...
            audit = LicenseService._create_audit_log(
                ...
                delta={"expired_at": now.isoformat(), "days_overdue": (now - license.ends_at).days},
                user_id=license.user_id,
                actor_type="system",
    ...
    def get_active_license(db: Session, user_id: int) -> Optional[License]:
        ...
        return (
            db.query(License)
            .filter(
                and_(License.user_id == user_id, License.status == "active", License.ends_at > now)
            )
            .first()
        )
    ...
    def validate_license_integrity(db: Session, license_id: int) -> bool:
        ...
        audit_logs = (
            db.query(LicenseAudit)
            .filter(LicenseAudit.license_id == license_id)
            .order_by(LicenseAudit.id.asc())
            .all()
        )
        ...
        for audit in audit_logs:
            if not audit.verify_hash_chain(previous):
                logger.error(f"Hash chain broken at audit {audit.id} for license {license_id}")
                return False
            previous = audit
