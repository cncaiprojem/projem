...
class EnvironmentService:
    ...
    async def _validate_service_configuration(self) -> None:
        ...
        if self._config_errors:
            raise RuntimeError(
                f"Critical configuration errors: {', '.join(self._config_errors)}"
            )
    ...
    async def _validate_development_configuration(self) -> None:
        ...
        if not environment.is_dev_mode and environment.DEV_AUTH_BYPASS:
            self._config_warnings.append(
                "DEV_AUTH_BYPASS enabled but DEV_MODE is disabled"
            )
    ...
    async def _validate_production_configuration(self) -> None:
        ...
        if not environment.should_force_https:
            self._config_warnings.append(
                "HTTPS enforcement is disabled in production environment"
            )
        
        if not environment.should_use_secure_cookies:
            self._config_errors.append(
                "Secure cookies must be enabled in production environment"
            )
        ...
        if environment.DEV_AUTH_BYPASS:
            self._config_errors.append(
                "Authentication bypass must be disabled in production"
            )
        ...
        if not environment.AUDIT_LOG_ENABLED:
            self._config_errors.append(
                "Audit logging must be enabled in production for compliance"
            )
    ...
    async def _validate_kvkv_compliance(self) -> None:
        ...
        if not environment.KVKV_AUDIT_LOG_ENABLED:
            self._config_errors.append(
                "KVKV audit logging must be enabled for Turkish compliance"
            )
    ...
    def get_environment_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dev_features": {
                ...
                "detailed_errors": environment.DEV_DETAILED_ERRORS if environment.is_development else False,
                "response_annotations": environment.DEV_RESPONSE_ANNOTATIONS if environment.is_development else False,
                "csrf_localhost_bypass": environment.CSRF_DEV_LOCALHOST_BYPASS if environment.is_development else False,
                "mock_external_services": environment.DEV_MOCK_EXTERNAL_SERVICES if environment.is_development else False
            },
            "production_hardening": {
                ...
                "disable_debug_endpoints": environment.PROD_DISABLE_DEBUG_ENDPOINTS if environment.is_production else False
            },
            "kvkv_compliance": {
    ...
    def is_feature_enabled(self, feature_name: str) -> bool:
        ...
        feature_map = {
            ...
            "response_annotations": environment.DEV_RESPONSE_ANNOTATIONS and environment.is_development,
            "csrf_localhost_bypass": environment.CSRF_DEV_LOCALHOST_BYPASS and environment.is_development,
