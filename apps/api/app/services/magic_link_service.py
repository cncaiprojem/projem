...
class MagicLinkResult:
    ...
    def __init__(
        self, user: User, access_token: str, refresh_token: str, expires_in: int, session_id: str
    ):
        self.user = user
...
class MagicLinkService:
    ...
    def _create_token(self, payload: Dict[str, Any]) -> str:
        ...
        # Add expiration
        payload["exp"] = int(
            (datetime.now(timezone.utc) + timedelta(minutes=self.token_expiry_minutes)).timestamp()
        )
        ...
        signature = hmac.new(
            self.secret_key.encode("utf-8"), message_b64.encode("utf-8"), hashlib.sha256
        ).hexdigest()
        ...
        try:
            ...
            if not self._check_rate_limit(db, email, ip_address):
                self._log_security_event(
                    ...
                    ip_address,
                    user_agent,
                    {"email_hash": hash(email) % 1000000, "reason": "rate_limit_exceeded"},
                )
            ...
            if user:
                ...
                self._log_audit_event(
                    ...
                    {"email_hash": hash(email) % 1000000, "token_length": len(magic_link_token)},
                )

                logger.info(
                    "Magic link requested for existing user",
                    extra={
                ...
                self._log_security_event(
                    db,
                    None,
                    "MAGIC_LINK_NONEXISTENT_USER",
                    ip_address,
                    user_agent,
                    {"email_hash": hash(email) % 1000000, "reason": "user_not_found"},
                )
    ...
    def consume_magic_link(
        ...
        try:
            ...
            magic_link = (
                db.query(MagicLink)
                .filter(MagicLink.nonce == nonce, MagicLink.email == email)
                .first()
            )

            if not magic_link:
                raise MagicLinkError("ERR-ML-INVALID", "Magic link geçersiz veya bulunamadı")
            ...
            if magic_link.is_consumed:
                ...
                raise MagicLinkError("ERR-ML-ALREADY-USED", "Magic link zaten kullanılmış")

            if magic_link.is_expired:
                magic_link.invalidate("expired")
                raise MagicLinkError("ERR-ML-EXPIRED", "Magic link süresi dolmuş")

            if not magic_link.is_valid:
                raise MagicLinkError("ERR-ML-INVALID", "Magic link geçersiz")

            # Find user
            user = db.query(User).filter(User.email == email).first()
            if not user:
                raise MagicLinkError("ERR-ML-USER-NOT-FOUND", "Kullanıcı bulunamadı")

            # Check user account status
            if not user.can_attempt_login():
                raise MagicLinkError("ERR-ML-ACCOUNT-LOCKED", "Hesap kilitli veya aktif değil")
            ...
            db.rollback()
            logger.error(
                "Magic link consumption failed",
                exc_info=True,
                extra={"operation": "consume_magic_link", "error_type": type(e).__name__},
            )
            raise MagicLinkError("ERR-ML-CONSUMPTION-FAILED", "Magic link kullanımı başarısız")
    ...
    def cleanup_expired_links(self, db: DBSession) -> int:
        ...
        """
        try:
            cutoff_time = datetime.now(timezone.utc) - timedelta(hours=self.cleanup_interval_hours)
            ...
            logger.info(
                "Magic links cleanup completed",
                extra={"operation": "cleanup_expired_links", "cleanup_count": cleanup_count},
            )
            ...
            db.rollback()
            logger.error(
                "Magic links cleanup failed",
                exc_info=True,
                extra={"operation": "cleanup_expired_links", "error_type": type(e).__name__},
            )
            return 0
    ...
    def _validate_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not all(key in payload for key in ["email", "nonce", "iat"]):
                raise MagicLinkError("ERR-ML-MALFORMED", "Magic link formatı geçersiz")
    ...
    def _check_rate_limit(self, db: DBSession, email: str, ip_address: Optional[str]) -> bool:
        """Check rate limiting for magic link requests."""
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=1)

        # Check email-based rate limit
        email_count = (
            db.query(func.count(MagicLink.id))
            .filter(MagicLink.email == email, MagicLink.issued_at > cutoff_time)
            .scalar()
        )
        ...
        if ip_address:
            ip_count = (
                db.query(func.count(MagicLink.id))
                .filter(MagicLink.ip_address == ip_address, MagicLink.issued_at > cutoff_time)
                .scalar()
            )
    ...
    def _send_magic_link_email(self, email: str, token: str) -> None:
        ...
        logger.info(
            "Magic link email would be sent",
            extra={
    ...
    def _log_security_event(
        ...
        try:
            security_event = SecurityEvent(
                ...
                details=details,
                severity="HIGH"
                if "REUSE" in event_type or "RATE_LIMITED" in event_type
                else "MEDIUM",
            )
