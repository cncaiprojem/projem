...
from .universal_importer import ImportOptions, UniversalImporter
...
class ConversionOptions(BaseModel):
    ...
    target_face_count: int | None = Field(default=None, ge=100, le=10000000, description="Hedef yüz sayısı")
...
class ConversionMatrix:
    ...
    def get_conversion_path(
        ...
        source: str,
        target: str
    ) -> tuple[ConversionMethod, str, str] | None:
        """
    ...
    async def _convert_indirect(
        ...
        try:
            ...
            for i in range(len(path) - 1):
                ...
                # Create temp file for intermediate results using tempfile module
                if i < len(path) - 2:
                    # Use tempfile.NamedTemporaryFile for secure temp file creation
                    suffix = f".{target}"
                    prefix = f"convert_{job_id}_{i}_"
                    fd, temp_path = await asyncio.to_thread(
                        tempfile.mkstemp, suffix=suffix, prefix=prefix
                    )
                    # Close the file descriptor immediately as we'll use the path
