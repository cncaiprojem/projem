...
from ..services.pii_masking_service import DataClassification, pii_masking_service
    ...
    async def register_user(
        ...
        try:
            ...
            strength_result = password_service.validate_password_strength(
                password, {"email": email, "full_name": full_name}
            )
            ...
            user = User(
                ...
                + timedelta(hours=self.email_verification_token_lifetime_hours),
                auth_metadata={
            ...
            raise AuthenticationError(
                "ERR-AUTH-REGISTRATION-FAILED", "Kayıt işlemi başarısız oldu, lütfen tekrar deneyin"
            )
    ...
    async def authenticate_user(
        ...
        try:
            ...
            password_valid = password_service.verify_password(
                password, user.password_hash, user.password_salt, user.password_algorithm
            )
            ...
            if user.password_must_change or user.is_password_expired():
                ...
                raise AuthenticationError(
                    "ERR-AUTH-PASSWORD-EXPIRED", "Şifre süresi dolmuş, lütfen yeni şifre belirleyin"
                )
            ...
            if user.requires_mfa() and user.mfa_enabled:
                ...
                await self._log_security_event(
                    db,
                    user.id,
                    "LOGIN_PASSWORD_SUCCESS_MFA_PENDING",
                    ip_address,
                    user_agent,
                    {
                        "email": self._mask_email(email),
            ...
            raise AuthenticationError(
                "ERR-AUTH-SYSTEM-ERROR", "Sistem hatası, lütfen tekrar deneyin"
            )
    ...
    async def reset_password(
        ...
        try:
            ...
            if not strength_result.ok:
                ...
                raise AuthenticationError(
                    "ERR-AUTH-WEAK-PASSWORD",
                    "Yeni şifre güvenlik gereksinimlerini karşılamıyor: "
                    + ", ".join(strength_result.feedback),
                )
            ...
            raise AuthenticationError(
                "ERR-AUTH-RESET-FAILED", "Şifre sıfırlama işlemi başarısız, lütfen tekrar deneyin"
            )
