...
class JWTService:
    ...
    def create_access_token(
        self, user: User, session: Session, scopes: Optional[List[str]] = None
    ) -> str:
        """
        ...
        try:
            ...
            logger.error(
                "JWT access token creation failed",
                exc_info=True,
                extra={
                    "operation": "jwt_create_access_token",
                    "user_id": user.id,
    ...
    def verify_access_token(self, token: str, db: DBSession) -> JWTClaims:
        """
        ...
        try:
            ...
            if not all([claims.sub, claims.role, claims.sid]):
                raise JWTError(JWTErrorCode.TOKEN_MISSING_CLAIMS, "Token'da gerekli bilgiler eksik")

            # Validate session exists and is active
            session = (
                db.query(Session)
                .filter(Session.id == uuid.UUID(claims.sid), Session.user_id == int(claims.sub))
                .first()
            )

            if not session:
                raise JWTError(JWTErrorCode.SESSION_NOT_FOUND, "Token'a bağlı oturum bulunamadı")

            if not session.is_active:
                raise JWTError(JWTErrorCode.TOKEN_REVOKED, "Token iptal edilmiş veya süresi dolmuş")
            ...
            raise JWTError(JWTErrorCode.TOKEN_EXPIRED, "Token'ın süresi dolmuş")
        except jwt.InvalidSignatureError:
            raise JWTError(JWTErrorCode.TOKEN_INVALID_SIGNATURE, "Token imzası geçersiz")
        except jwt.InvalidAlgorithmError:
            raise JWTError(JWTErrorCode.TOKEN_WRONG_ALGORITHM, "Token algoritması desteklenmiyor")
        except jwt.InvalidTokenError as e:
            raise JWTError(
                JWTErrorCode.TOKEN_MALFORMED, "Token formatı geçersiz", {"jwt_error": str(e)}
            )
        except JWTError:
            ...
            logger.error(
                ...
                exc_info=True,
                extra={"operation": "jwt_verify_access_token", "error_type": type(e).__name__},
            )
            raise JWTError(
                JWTErrorCode.TOKEN_INVALID,
    ...
    def revoke_tokens_for_session(
        self, db: DBSession, session_id: uuid.UUID, reason: str = "session_revoked"
    ) -> None:
        """
        ...
        try:
            ...
            if session and session.is_active:
                ...
                logger.info(
                    "JWT tokens revoked for session",
                    extra={
    ...
    def revoke_all_tokens_for_user(
        ...
        try:
            ...
            db.rollback()
            logger.error(
                "User JWT token revocation failed",
                exc_info=True,
                extra={
                    "operation": "jwt_revoke_all_user_tokens",
                    "user_id": user_id,
