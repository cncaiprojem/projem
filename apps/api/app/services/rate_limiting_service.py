...
class RateLimitPolicy:
    ...
    burst_threshold: Optional[int] = None  # For burst detection
    description: str = ""
...
class EnterpriseRateLimitingService:
    ...
    POLICIES: Dict[RateLimitType, RateLimitPolicy] = {
        ...
        RateLimitType.GENERAL: RateLimitPolicy(
            requests=100, window_seconds=60, key_type="ip", description="Genel API"
        ),
    }
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                "Enterprise rate limiting initialized",
                extra={
            ...
            logger.error(
                ...
                extra={"operation": "rate_limiting_init", "status": "failed", "error": str(e)},
            )
            raise
    ...
    async def close(self):
        ...
        try:
            ...
            logger.info(
                "Enterprise rate limiting closed",
                extra={"operation": "rate_limiting_close", "status": "success"},
            )

        except Exception as e:
            logger.error(
                "Failed to close rate limiting",
                extra={"operation": "rate_limiting_close", "status": "failed", "error": str(e)},
            )

    def get_client_ip(self, request: Request, trust_proxy: bool = True) -> Optional[str]:
        """
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            if not allowed:
                await self._check_brute_force_pattern(request, policy_type, db, user, current_count)

                # Log security event for rate limiting
                await self._log_rate_limit_event(request, policy_type, db, user, current_count)
    ...
    async def _check_brute_force_pattern(
        ...
        try:
            ...
            if policy.burst_threshold and current_count >= policy.burst_threshold:
            ...
            logger.error(
                ...
                extra={"operation": "brute_force_detection", "error": str(e)},
            )
    ...
    async def _log_rate_limit_event(
        ...
        try:
            ...
            logger.error(
                "Rate limit event logging failed",
                extra={"operation": "rate_limit_logging", "error": str(e)},
            )

    def create_rate_limit_exception(
        self, result: RateLimitResult, policy_type: RateLimitType
    ) -> HTTPException:
        ...
        message = error_messages.get(
            policy_type, "Hız sınırı aşıldı. Lütfen {retry_after} saniye sonra tekrar deneyin."
