...
from sqlalchemy import (
    BigInteger, Boolean, CheckConstraint, DateTime, Index, Integer, String, Text
)
...
class NotificationTemplate(Base, TimestampMixin):
    ...
    type: Mapped[NotificationTemplateType] = mapped_column(
        nullable=False,
        index=True,
        comment="Template type for specific use cases"
    )
    ...
    language: Mapped[str] = mapped_column(
        ...
        comment="Language code (tr-TR, en-US)"
    )
    ...
    description: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Template description for administration"
    )
    ...
    body_template: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="Email HTML or SMS text template"
    )
    
    plain_text_template: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Email plain text fallback"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "length(name) >= 3",
            name='ck_notification_templates_name_length'
        ),
        
        # Version must be positive
        CheckConstraint(
            "version >= 1",
            name='ck_notification_templates_version_positive'
        ),
        
        # Only one active template per type+channel+language
        Index(
            'uq_notification_templates_active',
            'type', 'channel', 'language', 'is_active',
            unique=True,
