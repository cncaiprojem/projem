...
from sqlalchemy import String, Integer, Index, Numeric, CheckConstraint, Enum as SQLEnum
...
class Material(Base, TimestampMixin):
    ...
    category: Mapped[MaterialCategory] = mapped_column(
        SQLEnum(MaterialCategory), nullable=False, index=True
    )
    name: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    grade: Mapped[Optional[str]] = mapped_column(String(50))

    # Physical properties
    density_g_cm3: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 3))
    ...
    cutting_speed_m_min: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 2))
    feed_rate_mm_tooth: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 4))

    # Additional properties
    properties: Mapped[Optional[dict]] = mapped_column(JSONB)

    # Cost information
    cost_per_kg: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 2))
    supplier: Mapped[Optional[str]] = mapped_column(String(255))

    # Constraints and indexes
    __table_args__ = (
        CheckConstraint("density_g_cm3 > 0", name="ck_materials_density_positive"),
        CheckConstraint("hardness_hb > 0", name="ck_materials_hardness_positive"),
        CheckConstraint("tensile_strength_mpa > 0", name="ck_materials_tensile_positive"),
        CheckConstraint(
            "machinability_rating >= 0 AND machinability_rating <= 100",
            name="ck_materials_machinability_valid",
        ),
        CheckConstraint("cutting_speed_m_min > 0", name="ck_materials_cutting_speed_positive"),
        CheckConstraint("feed_rate_mm_tooth > 0", name="ck_materials_feed_rate_positive"),
        CheckConstraint("cost_per_kg >= 0", name="ck_materials_cost_non_negative"),
        Index(
            "idx_materials_properties",
            "properties",
