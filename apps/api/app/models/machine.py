...
class Machine(Base, TimestampMixin):
    ...
    work_envelope_y_mm: Mapped[Decimal] = mapped_column(
        Numeric(10, 2),
        nullable=False
    )
    work_envelope_z_mm: Mapped[Decimal] = mapped_column(
        Numeric(10, 2),
        nullable=False
    )
    ...
    __table_args__ = (
        CheckConstraint('axes >= 3 AND axes <= 9',
                       name='ck_machines_axes_valid'),
        CheckConstraint('work_envelope_x_mm > 0',
                       name='ck_machines_envelope_x_positive'),
        CheckConstraint('work_envelope_y_mm > 0',
                       name='ck_machines_envelope_y_positive'),
        CheckConstraint('work_envelope_z_mm > 0',
                       name='ck_machines_envelope_z_positive'),
        CheckConstraint('spindle_max_rpm > 0',
                       name='ck_machines_spindle_rpm_positive'),
        CheckConstraint('spindle_power_kw > 0',
                       name='ck_machines_spindle_power_positive'),
        CheckConstraint('tool_capacity > 0',
                       name='ck_machines_tool_capacity_positive'),
        CheckConstraint('hourly_rate >= 0',
                       name='ck_machines_hourly_rate_non_negative'),
    ...
    def can_machine_part(self, x: float, y: float, z: float) -> bool:
        ...
        return (
            x <= float(self.work_envelope_x_mm) and
            y <= float(self.work_envelope_y_mm) and
            z <= float(self.work_envelope_z_mm)
        )
