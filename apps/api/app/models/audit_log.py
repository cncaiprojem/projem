...
class AuditLog(Base):
    ...
    __table_args__ = (
        ...
        # Performance indexes
        Index("idx_audit_logs_scope_created", "scope_type", "scope_id", "created_at"),
        ...
        Index("idx_audit_logs_session_id", "session_id", postgresql_where="session_id IS NOT NULL"),
    )
    ...
    scope_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True, index=True, comment="ID of the specific entity being audited"
    )
    ...
    resource: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="Resource being audited"
    )
    ...
    ua_masked: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="KVKV-compliant masked user agent"
    )

    payload: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Structured data about the audited event"
    )
    ...
    def verify_chain_integrity(self, prev_entry: Optional["AuditLog"]) -> bool:
        ...
        expected_chain_hash = self.compute_chain_hash(self.prev_chain_hash, self.payload)
