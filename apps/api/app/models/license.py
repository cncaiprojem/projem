...
from sqlalchemy import BigInteger, CheckConstraint, DateTime, String, Text, ForeignKey, Index, text
...
class License(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True, autoincrement=True, comment="Unique license identifier"
    )
    ...
    type: Mapped[str] = mapped_column(
        String(10), nullable=False, comment="License duration type: 3m, 6m, or 12m"
    )
    ...
    status: Mapped[str] = mapped_column(
        String(20), nullable=False, index=True, comment="License status: active, expired, canceled"
    )

    # Cancellation reason (nullable for active/expired licenses)
    reason: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Reason for cancellation (if canceled)"
    )
    ...
    canceled_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="When license was canceled (if applicable)"
    )
    ...
    invoices: Mapped[list["Invoice"]] = relationship(
        "Invoice", back_populates="license", lazy="select", order_by="desc(Invoice.issued_at)"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint("type IN ('3m', '6m', '12m')", name="ck_licenses_valid_type"),
        # Status validation
        CheckConstraint(
        ...
        Index(
            "idx_licenses_active_expiring", "ends_at", postgresql_where=text("status = 'active'")
        ),
        # JSONB index for scope queries
        Index("idx_licenses_scope", "scope", postgresql_using="gin"),
    )
