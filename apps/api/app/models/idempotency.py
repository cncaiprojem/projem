...
from sqlalchemy import Column, String, Integer, DateTime, JSON, UniqueConstraint, Index, Text
...
class IdempotencyRecord(Base):
    ...
    user_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), nullable=False, index=True, comment="User who made the request"
    )

    # Idempotency key from header
    idempotency_key: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True, comment="Unique key from Idempotency-Key header"
    )
    ...
    method: Mapped[str] = mapped_column(
        String(10), nullable=False, comment="HTTP method (POST, PUT, etc.)"
    )

    # Response storage
    response_status: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="HTTP response status code"
    )

    response_data: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, comment="Stored response data for replay"
    )
    ...
    __table_args__ = (
        ...
        Index("ix_idempotency_expires", "expires_at", postgresql_where="expires_at > NOW()"),
