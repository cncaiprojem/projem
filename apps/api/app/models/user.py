...
from .enums import Locale, UserRole
...
class User(Base, TimestampMixin):
    ...
    last_lockout_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
    ...
    security_preferences: Mapped[dict | None] = mapped_column(
        JSONB,
        comment='User security preferences and settings'
    )
    auth_metadata: Mapped[dict | None] = mapped_column(
        JSONB,
    ...
    tax_no: Mapped[str | None] = mapped_column(
        String(20),
        index=True
    ...
    user_metadata: Mapped[dict | None] = mapped_column(JSONB, name="metadata")
    ...
    def is_password_expired(self, max_age_days: int = 90) -> bool:
        ...
        return datetime.now(UTC) - self.password_updated_at > max_age
    ...
    def increment_failed_login_attempts(self) -> None:
        ...
        if self.failed_login_attempts >= 10:
            self.account_locked_until = datetime.now(UTC) + timedelta(minutes=15)
            self.last_lockout_at = datetime.now(UTC)

    def reset_failed_login_attempts(self) -> None:
        """Reset failed login attempts after successful login."""
        self.failed_login_attempts = 0
        self.account_locked_until = None
        self.last_successful_login_at = datetime.now(UTC)
