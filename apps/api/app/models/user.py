...
class User(Base, TimestampMixin):
    ...
    password_salt: Mapped[Optional[str]] = mapped_column(
        String(64), comment="Per-user salt for Argon2"
    )
    password_algorithm: Mapped[str] = mapped_column(
        String(20), nullable=False, default="argon2id", comment="Password hashing algorithm"
    )
    password_updated_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Last password change timestamp"
    )
    ...
    role: Mapped[UserRole] = mapped_column(
        SQLEnum(UserRole), nullable=False, default=UserRole.ENGINEER, index=True
    )

    # Account lockout mechanism
    failed_login_attempts: Mapped[int] = mapped_column(
        nullable=False, default=0, comment="Current failed login attempt count"
    )
    account_locked_until: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Account lockout expiration timestamp"
    )
    last_failed_login_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Timestamp of last failed login"
    )
    last_lockout_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Timestamp when account was last locked"
    )

    # Authentication audit fields
    last_successful_login_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Timestamp of last successful login"
    )
    last_login_ip: Mapped[Optional[str]] = mapped_column(
        String(45), comment="IP address of last login (IPv4/IPv6)"
    )
    last_login_user_agent: Mapped[Optional[str]] = mapped_column(
        Text, comment="User agent of last login"
    )
    total_login_count: Mapped[int] = mapped_column(
        BigInteger, nullable=False, default=0, comment="Total successful login count"
    )

    # Email verification and security
    email_verified_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Email verification timestamp"
    )
    email_verification_token: Mapped[Optional[str]] = mapped_column(
        String(128), comment="Current email verification token"
    )
    email_verification_expires_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Email verification token expiration"
    )

    # Password reset functionality
    password_reset_token: Mapped[Optional[str]] = mapped_column(
        String(128), comment="Current password reset token"
    )
    ...
    password_reset_attempts: Mapped[int] = mapped_column(
        nullable=False, default=0, comment="Password reset attempt count in current window"
    )
    ...
    deactivated_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Account deactivation timestamp"
    )
    deactivation_reason: Mapped[Optional[str]] = mapped_column(
        String(100), comment="Reason for account deactivation"
    )

    # Turkish KVKV compliance fields
    data_processing_consent: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="KVKV data processing consent"
    )
    data_processing_consent_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="KVKV consent timestamp"
    )
    marketing_consent: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="Marketing communication consent"
    )
    marketing_consent_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="Marketing consent timestamp"
    )

    # Multi-Factor Authentication (MFA) fields for Task 3.7
    mfa_enabled: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="MFA activation status"
    )
    mfa_secret_encrypted: Mapped[Optional[str]] = mapped_column(
        String(512), comment="Encrypted MFA TOTP secret (AES-256-GCM)"
    )
    mfa_enabled_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="MFA activation timestamp"
    )
    mfa_backup_codes_count: Mapped[int] = mapped_column(
        nullable=False, default=0, comment="Number of unused backup codes remaining"
    )

    # Security preferences and metadata
    security_preferences: Mapped[Optional[dict]] = mapped_column(
        JSONB, comment="User security preferences and settings"
    )
    auth_metadata: Mapped[Optional[dict]] = mapped_column(
        JSONB, comment="Authentication-related metadata"
    )
    ...
    # Preferences
    locale: Mapped[Locale] = mapped_column(SQLEnum(Locale), nullable=False, default=Locale.TR)
    ...
    models: Mapped[List["Model"]] = relationship("Model", back_populates="user")
    ...
    audit_logs: Mapped[List["AuditLog"]] = relationship(
        "AuditLog", back_populates="actor_user", foreign_keys="AuditLog.actor_user_id"
    )
    ...
    oidc_accounts: Mapped[List["OIDCAccount"]] = relationship(
        "OIDCAccount", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_users_last_login_ip", "last_login_ip", postgresql_where="last_login_ip IS NOT NULL"
        ),
        Index("idx_users_password_updated_at", "password_updated_at"),
        Index("idx_users_full_name", "full_name", postgresql_where="full_name IS NOT NULL"),
        Index("idx_users_data_processing_consent", "data_processing_consent"),
        Index("idx_users_mfa_enabled", "mfa_enabled"),
        Index(
            "idx_users_mfa_enabled_at",
            "mfa_enabled_at",
            postgresql_where="mfa_enabled_at IS NOT NULL",
        ),
        CheckConstraint(
            "failed_login_attempts >= 0", name="ck_users_failed_login_attempts_non_negative"
        ),
        CheckConstraint("total_login_count >= 0", name="ck_users_total_login_count_non_negative"),
        CheckConstraint(
            "password_reset_attempts >= 0", name="ck_users_password_reset_attempts_non_negative"
        ),
        CheckConstraint(
    ...
    def has_permission(self, permission: str) -> bool:
        ...
        role_permissions = {
            ...
            UserRole.VIEWER: {"model.read", "job.read", "cam.read", "simulation.read"},
        }
    ...
    def can_attempt_login(self) -> bool:
        ...
        return (
            self.account_status == "active" and not self.is_account_locked()
