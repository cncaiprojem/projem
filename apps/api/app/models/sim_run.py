...
class SimRun(Base, TimestampMixin):
    ...
    # Relationships
    job: Mapped["Job"] = relationship("Job", back_populates="sim_runs", foreign_keys=[job_id])

    # Enterprise-grade indexing strategy
    __table_args__ = (
        Index("idx_sim_runs_job_id_status", "job_id", "status"),
        Index("idx_sim_runs_status", "status", postgresql_where="status IN ('pending', 'running')"),
        Index("idx_sim_runs_created_at", "created_at"),
    )

    def __repr__(self) -> str:
        """Developer-friendly representation."""
        return f"<SimRun(id={self.id}, job_id={self.job_id}, status={self.status.value})>"
    ...
    def add_collision_results(
        self, collision_count: int, collision_details: dict, severity_level: str
    ) -> None:
    ...
    def add_performance_metrics(
        self, simulation_time_ms: int, accuracy_percentage: float, memory_usage_mb: float
    ) -> None:
