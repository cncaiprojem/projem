...
class NotificationAttempt(Base):
    ...
    id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True, autoincrement=True, comment="Unique attempt identifier"
    )
    ...
    attempt_number: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="Sequential attempt number (1, 2, 3, etc.)"
    )

    provider: Mapped[NotificationProvider] = mapped_column(
        nullable=False, comment="Provider used for this attempt"
    )

    status: Mapped[NotificationStatus] = mapped_column(
        nullable=False, index=True, comment="Result status of this attempt"
    )

    # Provider interaction details
    provider_request: Mapped[Optional[dict]] = mapped_column(
        JSONB, nullable=True, comment="Request payload sent to provider"
    )

    provider_response: Mapped[Optional[dict]] = mapped_column(
        JSONB, nullable=True, comment="Response received from provider"
    )

    provider_message_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, index=True, comment="Provider-specific message ID"
    )

    # Error details
    error_code: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, index=True, comment="Provider-specific error code"
    )
    ...
    http_status_code: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="HTTP status code from provider API"
    )
    ...
    completed_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="When attempt completed"
    )

    duration_ms: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="Attempt duration in milliseconds"
    )

    # Relationships
    notification: Mapped["NotificationDelivery"] = relationship(
        "NotificationDelivery", back_populates="attempts", lazy="select"
    )

    # Ultra-enterprise constraints and indexes
    __table_args__ = (
        # Attempt number must be positive
        CheckConstraint("attempt_number >= 1", name="ck_notification_attempts_positive_attempt"),
        ...
        UniqueConstraint(
            "notification_id", "attempt_number", name="uq_notification_attempts_number"
        ),
        # Performance indexes
    ...
    def set_provider_response(
        self, response_data: dict, message_id: str = None, http_status: int = None
    ) -> None:
    ...
    @classmethod
    def create_attempt(
        cls, notification_id: int, attempt_number: int, provider: NotificationProvider
    ) -> "NotificationAttempt":
    ...
    def get_attempt_stats(cls, db_session, hours: int = 24) -> dict:
        ...
        # Get overall stats
        total_attempts = (
            db_session.query(func.count(cls.id)).filter(cls.started_at >= since).scalar()
        )
        ...
        error_stats = (
            db_session.query(cls.error_code, func.count(cls.id).label("count"))
            .filter(and_(cls.started_at >= since, cls.error_code.is_not(None)))
            .group_by(cls.error_code)
        ...
        return {
            ...
            "top_errors": [
                {"error_code": stat.error_code, "count": stat.count} for stat in error_stats
            ],
        }
