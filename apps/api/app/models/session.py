...
class Session(Base, TimestampMixin):
    ...
    parent_session: Mapped[Optional["Session"]] = relationship(
        "Session",
        remote_side=[id],
        back_populates="rotated_sessions"
    )
    ...
    __table_args__ = (
        ...
        Index('idx_sessions_expires_active', 'expires_at',
              postgresql_where='revoked_at IS NULL'),
        ...
        Index('idx_sessions_rotation_chain', 'rotated_from',
        ...
        CheckConstraint(
            'LENGTH(refresh_token_hash) = 128',
            name='ck_sessions_refresh_token_hash_length'
        ),
        ...
        CheckConstraint(
            ...
            name='ck_sessions_expires_after_created'
        ),
        ...
        CheckConstraint(
            'rotated_from IS NULL OR rotated_from != id',
            name='ck_sessions_no_self_rotation'
        ),
    )
    ...
    def extend_expiration(self, extension_hours: int = 168) -> None:
        ...
        """
        if self.is_active:
            self.expires_at = datetime.now(UTC) + timedelta(hours=extension_hours)
