...
class Payment(Base, TimestampMixin):
    ...
    provider: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Payment provider identifier (stripe, iyzico, etc.)"
    )

    provider_payment_id: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Provider payment intent ID"
    )

    # Financial details with cent precision - Task 4.6 specification
    amount_cents: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="Payment amount in smallest currency unit (cents)"
    )
    ...
    raw_request: Mapped[Optional[dict]] = mapped_column(
        JSONB, nullable=True, comment="Raw provider request data"
    )

    raw_response: Mapped[Optional[dict]] = mapped_column(
        JSONB, nullable=True, comment="Raw provider response data"
    )

    # Relationships
    invoice: Mapped[Invoice] = relationship("Invoice", back_populates="payments", lazy="select")
    ...
    __table_args__ = (
        ...
        # Currency constraint
        CheckConstraint("currency = 'TRY'", name="ck_payments_currency_try_only"),
    ...
    def __str__(self) -> str:
        return (
            f"Payment {self.provider_payment_id}: {self.amount_decimal:.2f} {self.currency.value}"
        )
...
class PaymentWebhookEvent(Base, TimestampMixin):
    ...
    event_id: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Unique provider event ID for idempotency"
    )

    provider: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Payment provider identifier"
    )

    event_type: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Event type (payment_intent.succeeded, etc.)"
    )
    ...
    processed: Mapped[bool] = mapped_column(
        nullable=False, server_default=text("false"), comment="Whether event was processed"
    )

    processed_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="When event was processed"
    )

    # Raw event data
    raw_event: Mapped[dict] = mapped_column(JSONB, nullable=False, comment="Raw webhook event data")

    # Relationships
    payment: Mapped[Optional[Payment]] = relationship("Payment", lazy="select")
    ...
    __table_args__ = (
        ...
        # Performance indexes
        Index("idx_webhook_events_provider_processed", "provider", "processed"),
...
class PaymentAuditLog(Base):
    ...
    actor_type: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Who performed action (system, webhook, user)"
    )

    actor_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="Actor identifier"
    )

    # Event context
    context: Mapped[Optional[dict]] = mapped_column(
        JSONB, nullable=True, comment="Additional context and metadata"
    )
    ...
    __table_args__ = (
        # Performance indexes
        Index("idx_audit_logs_payment_action", "payment_id", "action"),
