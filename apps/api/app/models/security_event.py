...
class SecurityEvent(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_security_events_user_type", "user_id", "type", "created_at"),
        Index(
    ...
    id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True, autoincrement=True, comment="Unique security event identifier"
    )
    ...
    session_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, index=True, comment="Session ID for user session tracking"
    )
    ...
    resource: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="Resource being accessed or affected"
    )
    ...
    ua_masked: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="KVKV-compliant masked user agent string"
    )
    ...
    def create_login_failed(
        ...
        """
        return cls(user_id=user_id, type="LOGIN_FAILED", ip_masked=ip_masked, ua_masked=ua_masked)

    @classmethod
    def create_access_denied(
        cls, user_id: int, ip_masked: Optional[str] = None, ua_masked: Optional[str] = None
    ) -> "SecurityEvent":
        ...
        Returns:
            SecurityEvent instance
        """
        return cls(user_id=user_id, type="ACCESS_DENIED", ip_masked=ip_masked, ua_masked=ua_masked)
    ...
    def is_suspicious(self) -> bool:
        ...
        return self.type.startswith("SUSPICIOUS_") or self.type in [
            "BRUTE_FORCE_DETECTED",
            "RATE_LIMIT_EXCEEDED",
            "UNUSUAL_LOCATION",
            "MULTIPLE_DEVICES",
