...
from sqlalchemy import String, Boolean, DateTime, ForeignKey, Index, CheckConstraint, Text
...
class MFABackupCode(Base, TimestampMixin):
    ...
    code_hash: Mapped[str] = mapped_column(
        String(64), nullable=False, comment="SHA-256 hash of backup code"
    )
    ...
    is_used: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="Whether this backup code has been used"
    )
    used_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), comment="When this code was used (if used)"
    )
    used_from_ip: Mapped[Optional[str]] = mapped_column(
        String(45), comment="IP address where code was used (IPv4/IPv6)"
    )
    used_user_agent: Mapped[Optional[str]] = mapped_column(
        Text, comment="User agent when code was used"
    )
    ...
    __table_args__ = (
        ...
        Index("idx_mfa_backup_codes_used_at", "used_at", postgresql_where="used_at IS NOT NULL"),
        Index(
            "idx_mfa_backup_codes_active",
            "user_id",
            "is_used",
            "expires_at",
        ...
        CheckConstraint(
            "expires_at > created_at", name="ck_mfa_backup_codes_expires_after_creation"
        ),
        CheckConstraint(
