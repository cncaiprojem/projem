...
from sqlalchemy import String, DateTime, Boolean, Index, CheckConstraint, Text
...
class MagicLink(Base, TimestampMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, comment="Unique magic link ID"
    )

    # Target email (may or may not correspond to existing user)
    email: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True, comment="Target email address for magic link"
    )
    ...
    consumed_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, index=True, comment="Token consumption timestamp"
    )

    # Security audit fields
    ip_address: Mapped[Optional[str]] = mapped_column(
        INET, nullable=True, comment="IP address of token request"
    )

    user_agent: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="User agent of token request"
    )

    # Device fingerprint for security correlation
    device_fingerprint: Mapped[Optional[str]] = mapped_column(
        String(512), nullable=True, comment="Device fingerprint for consumption verification"
    )

    # Consumption audit fields
    consumed_ip_address: Mapped[Optional[str]] = mapped_column(
        INET, nullable=True, comment="IP address of token consumption"
    )

    consumed_user_agent: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="User agent of token consumption"
    )

    consumed_device_fingerprint: Mapped[Optional[str]] = mapped_column(
        String(512), nullable=True, comment="Device fingerprint of token consumption"
    )

    # Invalidation tracking
    invalidated_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="Token invalidation timestamp"
    )

    invalidation_reason: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="Reason for token invalidation"
    )

    # Attempt tracking for security monitoring
    consumption_attempts: Mapped[int] = mapped_column(
        nullable=False, default=0, comment="Number of consumption attempts"
    )

    # Security metadata
    security_metadata: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Additional security metadata as JSON"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_magic_links_consumed_at", "consumed_at", postgresql_where="consumed_at IS NOT NULL"
        ),
        Index(
            "idx_magic_links_ip_address", "ip_address", postgresql_where="ip_address IS NOT NULL"
        ),
        Index(
