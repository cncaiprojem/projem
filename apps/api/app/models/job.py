...
class Job(Base, TimestampMixin):
    ...
    # Job type and status
    type: Mapped[JobType] = mapped_column(SQLEnum(JobType), nullable=False, index=True)
    ...
    artefacts: Mapped[List["Artefact"]] = relationship(
        "Artefact", back_populates="job", cascade="all, delete-orphan"
    )
    cam_runs: Mapped[List["CamRun"]] = relationship("CamRun", back_populates="job")
    sim_runs: Mapped[List["SimRun"]] = relationship("SimRun", back_populates="job")

    # Constraints and indexes (Task 2.3 requirements)
    __table_args__ = (
        CheckConstraint("progress >= 0 AND progress <= 100", name="ck_jobs_progress_valid"),
