...
def check_db() -> bool:
    ...
    try:
        ...
        logger.error(
            ...
            exc_info=True,
            extra={"operation": "check_db", "error_type": type(e).__name__},
        )
        return False
...
async def check_redis(redis_client: Optional[redis.Redis]) -> bool:
    ...
    try:
        ...
        logger.error(
            ...
            exc_info=True,
            extra={"operation": "check_redis", "error_type": type(e).__name__},
        )
        return False
...
async def create_redis_client() -> redis.Redis:
    ...
    try:
        ...
        logger.error(
            ...
            exc_info=True,
            extra={"operation": "create_redis_client", "error_type": type(e).__name__},
        )
        raise
...
async def close_redis_client(redis_client: Optional[redis.Redis]) -> None:
    ...
    if redis_client:
        try:
            ...
            logger.info(
                "Redis client closed successfully", extra={"operation": "close_redis_client"}
            )
        except Exception as e:
            logger.error(
                "Error closing Redis client",
                exc_info=True,
                extra={"operation": "close_redis_client", "error_type": type(e).__name__},
            )
...
def get_redis(request: Request) -> redis.Redis:
    ...
    if redis_client is None:
        logger.error("Redis client not initialized in app state", extra={"operation": "get_redis"})
        raise RuntimeError(
            "Redis client not initialized. Ensure startup event handler is configured."
        )
