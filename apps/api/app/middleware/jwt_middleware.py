...
class JWTAuthenticationError(HTTPException):
    ...
    def __init__(self, code: str, message: str, details: Optional[dict] = None):
        ...
        super().__init__(
            ...
            detail={"error_code": code, "message": message, "details": self.details},
        )
...
def get_current_user_optional(
    ...
    try:
        ...
        logger.warning(
            "Optional JWT authentication failed",
            extra={
...
def get_current_user(
    ...
    if not credentials:
        raise JWTAuthenticationError(
            JWTErrorCode.TOKEN_INVALID, "Authorization header gerekli. Bearer token bulunamadı."
        )
...
def require_scopes(*required_scopes: str) -> Callable:
    ...
    def scope_dependency(
        ...
        if not current_user.has_all_scopes(list(required_scopes)):
            logger.warning(
                "Insufficient scopes for endpoint access",
                extra={
            ...
            raise JWTAuthenticationError(
                ...
                {"required_scopes": list(required_scopes), "user_scopes": current_user.scopes},
            )
...
def require_admin() -> Callable:
    ...
    def admin_dependency(
        ...
        if not current_user.is_admin():
            logger.warning(
                "Admin access denied for non-admin user",
                extra={
...
def _authenticate_user(token: str, db: DBSession) -> AuthenticatedUser:
    ...
    try:
        ...
        if not user:
            raise JWTAuthenticationError(
                "ERR-USER-NOT-FOUND", "Token'da belirtilen kullanıcı bulunamadı"
            )

        # Verify user is still active
        if not user.is_active:
            raise JWTAuthenticationError("ERR-USER-INACTIVE", "Kullanıcı hesabı aktif değil")
        ...
        if not session:
            raise JWTAuthenticationError(
                JWTErrorCode.SESSION_NOT_FOUND, "Token'a bağlı oturum bulunamadı"
            )
        ...
        raise JWTAuthenticationError(
            "ERR-AUTH-FAILED", "Kimlik doğrulama başarısız", {"error_type": type(e).__name__}
        )
...
def get_admin_user(current_user: AuthenticatedUser = Depends(require_admin())) -> AuthenticatedUser:
