...
class CorrelationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        masked_ip = pii_masking_service.mask_ip_address(client_ip, MaskingLevel.MEDIUM) if client_ip else None
        masked_ua = pii_masking_service.mask_user_agent(user_agent, MaskingLevel.LIGHT) if user_agent else None
        ...
        try:
            ...
            if self.log_requests:
                self._log_request_completion(request, response, correlation_id, start_time, "success")
            ...
            # Log failed request
            if self.log_requests:
                self._log_request_completion(request, None, correlation_id, start_time, "error", str(e))
    ...
    def _extract_session_from_jwt(self, jwt_token: str) -> str | None:
