...
class RBACError(HTTPException):
    ...
    def _log_security_event(self, request: Request, user_id: int, code: str, message: str):
        ...
        try:
            ...
            logger.error(
                "Failed to log RBAC security event",
                exc_info=True,
                extra={
...
class RBACService:
    ...
    def check_role_permission(
        ...
        if not self.permissions.is_role_higher_or_equal(user_role, required_role):
            raise RBACError(
                ...
                {"user_role": user_role.value, "required_role": required_role.value},
                user_id,
                request,
            )
    ...
    def check_admin_permission(
        self, user_role: UserRole, user_id: int, request: Optional[Request] = None
    ) -> None:
        """
