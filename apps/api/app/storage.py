...
def upload_file(path: Path, key: str, bucket: str = None) -> str:
    ...
    try:
        ...
        logger.warning(
            "New S3 service failed, falling back to legacy method",
            error=str(e),
            path=str(path),
            key=key,
        )
...
def presigned_url(key: str, expires: int = 3600, bucket: str = None) -> Optional[str]:
    ...
    try:
        ...
        url = s3_service.generate_presigned_download_url(
            bucket=bucket, object_key=key, expiry=timedelta(seconds=expires)
        )
        return url
    except Exception as e:
        logger.warning(
            "New S3 service failed for presigned URL, falling back to legacy method",
            error=str(e),
            key=key,
            bucket=bucket,
        )
...
def presigned_upload_url(
    key: str, expires: int = 3600, bucket: str = None, content_type: str = None
) -> Optional[str]:
    """
    ...
    try:
        ...
        logger.error(
            "Failed to generate presigned upload URL", error=str(e), key=key, bucket=bucket
        )
        return None
...
def delete_file(key: str, bucket: str = None) -> bool:
    ...
    try:
        ...
        logger.error("Failed to delete file", error=str(e), key=key, bucket=bucket)
        return False
...
def list_files(prefix: str = None, bucket: str = None) -> list:
    ...
    try:
        ...
        logger.error("Failed to list files", error=str(e), prefix=prefix, bucket=bucket)
        return []
