...
async def start_google_auth(
    ...
    redirect_to: Optional[str] = Query(
        None, description="Kimlik doğrulama sonrası yönlendirilecek URL", max_length=2048
    ),
    ...
    **Rate Limiting:** 10 requests per minute per IP
    ...
    try:
        ...
        logger.warning(
            "OIDC authentication start failed",
            extra={
                "operation": "start_google_auth",
                "error_code": e.code,
                "client_ip": client_info["ip_address"],
        ...
        return create_oidc_error_response_json(
            OIDCServiceError("ERR-OIDC-AUTH-URL-FAILED", "OIDC yetkilendirme URL'si oluşturulamadı")
        )
...
async def google_auth_callback(
    ...
    **Rate Limiting:** 20 requests per minute per IP
    ...
    try:
        ...
        logger.error(
            "Unexpected OIDC callback error",
            exc_info=True,
            extra={
                "operation": "google_auth_callback",
                "client_ip": client_info["ip_address"],
        ...
        return create_oidc_error_response_json(
            OIDCServiceError("ERR-OIDC-AUTH-FAILED", "OIDC kimlik doğrulama başarısız")
        )
    ...
    try:
        ...
        if refresh_token:
            ...
            token_service.revoke_refresh_token(
                db=db, refresh_token=refresh_token, reason="oidc_user_logout"
            )
