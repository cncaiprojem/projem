...
router = APIRouter(
    ...
    dependencies=[
        Depends(require_scopes("admin:users"))
...
def list_users(
    ...
    search: Optional[str] = Query(
        None, min_length=2, max_length=100, description="Email veya isim araması"
    ),
):
    """
...
def update_user_role(
    ...
    if user_id == current_user.user_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Kendi rolünüzü değiştiremezsiniz"
        )
...
def list_security_events(
    ...
    # Log admin action
    logger.info(
        "Admin viewed security events",
        extra={
            "operation": "admin_list_security_events",
            "admin_user_id": current_user.user_id,
            "filters": {"user_id": user_id, "event_type": event_type, "hours": hours},
...
def get_security_events_summary(
    ...
    # Log admin action
    logger.info(
        "Admin viewed security events summary",
        extra={
...
def get_system_permissions(
    ...
    # Log admin action
    logger.info(
        "Admin viewed system permissions",
        extra={
...
def get_user_statistics(
    ...
    locked_users = (
        ...
        .filter(User.account_locked_until > datetime.now(timezone.utc))
        .scalar()
    )
