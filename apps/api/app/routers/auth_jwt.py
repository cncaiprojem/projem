...
async def refresh_access_token(
    ...
    try:
        ...
        if not refresh_token:
            logger.warning("Refresh token missing in request", extra={
        ...
        logger.info("Access token refreshed successfully", extra={
            ...
            'old_session_id': str(result.session.rotated_from) if result.session.rotated_from else None,
    ...
    try:
        ...
        for session in active_sessions:
            session_data = {
                ...
                'device_fingerprint': session.device_fingerprint[:20] + '...' if session.device_fingerprint else None,
