...
def analyze(
    body: DesignBrief, current_user: AuthenticatedUser = Depends(require_scopes("designs:read"))
):
    ...
    if not text:
        qs.append(
            DesignAnalysisQuestion(id="brief.prompt", text="Lütfen kısa bir tasarım tanımı girin.")
        )
...
def get(job_id: int, current_user: AuthenticatedUser = Depends(require_scopes("designs:read"))):
