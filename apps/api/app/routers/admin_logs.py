...
class AuditLogFilter(BaseModel):
    ...
    correlation_id: Optional[str] = Field(None, description="İstek korelasyon ID'si ile filtreleme")
    user_id: Optional[int] = Field(None, description="Kullanıcı ID'si ile filtreleme")
    event_type: Optional[str] = Field(None, description="Olay türü ile filtreleme (kısmi eşleşme)")
    scope_type: Optional[str] = Field(
        None, description="Kapsam türü ile filtreleme (user, job, financial, etc.)"
    )
    start_date: Optional[datetime] = Field(
        None, description="Başlangıç tarihi (ISO 8601 formatında)"
    )
    end_date: Optional[datetime] = Field(None, description="Bitiş tarihi (ISO 8601 formatında)")
...
class SecurityEventFilter(BaseModel):
    ...
    correlation_id: Optional[str] = Field(None, description="İstek korelasyon ID'si ile filtreleme")
    user_id: Optional[int] = Field(None, description="Kullanıcı ID'si ile filtreleme")
    event_type: Optional[str] = Field(None, description="Güvenlik olayı türü ile filtreleme")
    severity_filter: Optional[List[str]] = Field(
        None, description="Önem derecesi ile filtreleme (info, low, medium, high, critical)"
    )
...
async def verify_admin_access(
    ...
    has_audit_permission = await rbac_service.user_has_permission(db, current_user.id, "audit:read")
...
async def get_audit_logs(
    request: Request,
    correlation_id: Optional[str] = Query(None, description="Korelasyon ID'si ile filtreleme"),
    user_id: Optional[int] = Query(None, description="Kullanıcı ID'si ile filtreleme"),
    event_type: Optional[str] = Query(None, description="Olay türü ile filtreleme"),
    scope_type: Optional[str] = Query(None, description="Kapsam türü ile filtreleme"),
    ...
    limit: int = Query(100, ge=1, le=1000, description="Sayfa başına maksimum kayıt sayısı"),
    ...
    try:
        ...
        audit_logs = [AuditLogResponse(**log) for log in result["logs"]]
...
async def get_security_events(
    ...
    correlation_id: Optional[str] = Query(None, description="Korelasyon ID'si ile filtreleme"),
    user_id: Optional[int] = Query(None, description="Kullanıcı ID'si ile filtreleme"),
    event_type: Optional[str] = Query(None, description="Güvenlik olayı türü ile filtreleme"),
    start_date: Optional[datetime] = Query(None, description="Başlangıç tarihi (ISO 8601)"),
    end_date: Optional[datetime] = Query(None, description="Bitiş tarihi (ISO 8601)"),
    severity_filter: Optional[List[str]] = Query(None, description="Önem derecesi filtresi"),
    limit: int = Query(100, ge=1, le=1000, description="Sayfa başına maksimum kayıt sayısı"),
    ...
    try:
        ...
        # Convert to response format
        security_events = [SecurityEventResponse(**event) for event in result["events"]]
        ...
        return {
            ...
            "summary": {
                ...
                "time_span": {
                    ...
                    if audit_result["logs"] or security_result["events"]
                    else None,
                    "latest": max(
                        [log["created_at"] for log in audit_result["logs"]]
                        + [event["created_at"] for event in security_result["events"]]
...
async def get_security_analytics(
    ...
    user_id: Optional[int] = Query(None, description="Belirli kullanıcı için analiz"),
    db: Session = Depends(get_db),
    admin_user: User = Depends(verify_admin_access),
) -> Dict[str, Any]:
    ...
    try:
        ...
        analytics = await security_event_service.analyze_security_trends(
            db=db, time_window_hours=time_window_hours, user_id=user_id
        )

        # Verify audit chain integrity for the same period
        audit_verification = await audit_service.verify_audit_chain_integrity(db=db, limit=1000)
        ...
        return {
            ...
            "generated_by": {"user_id": admin_user.id, "correlation_id": get_correlation_id()},
        }
...
async def audit_system_health(
    db: Session = Depends(get_db), admin_user: User = Depends(verify_admin_access)
) -> Dict[str, Any]:
    """Check health status of audit and security systems."""

    try:
        # Check recent audit log creation
        recent_audit = await audit_service.get_audit_logs(db=db, limit=1)

        # Check recent security events
        recent_security = await security_event_service.get_security_events(db=db, limit=1)
        ...
        logger.error("audit_health_check_failed", error=str(e), user_id=admin_user.id)
