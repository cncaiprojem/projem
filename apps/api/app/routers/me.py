...
class AccountDeactivationRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"reason": "Artık sistemi kullanmıyorum", "confirm_deactivation": True}
        }
        ...
        session_list.append(
            {
                ...
                if session.user_agent and len(session.user_agent) > 100
...
def revoke_user_session(
    ...
    if not session:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Oturum bulunamadı veya size ait değil"
        )

    # Prevent user from revoking their current session
    if session.id == current_user.session_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Mevcut oturumunuzu sonlandıramazsınız"
        )
    ...
    return {"message": "Oturum başarıyla sonlandırıldı", "session_id": session_id}
...
def deactivate_account(
    ...
    if not deactivation.confirm_deactivation:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Hesap deaktivasyonu onaylanmalıdır"
        )
