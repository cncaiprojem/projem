...
async def handle_csp_violation(
    request: Request, report_data: Dict[str, Any], db: Session = Depends(get_db)
) -> JSONResponse:
    ...
    try:
        ...
        try:
            ...
            return JSONResponse(
                status_code=400, content={"status": "error", "message": "Invalid CSP report format"}
            )
        ...
        try:
            ...
            logger.error(
                ...
                extra={"operation": "csp_db_error", "error": str(db_error)},
            )
            db.rollback()
        ...
        logger.error(
            ...
            exc_info=True,
            extra={"operation": "csp_handler_error", "error_type": type(e).__name__},
        )
...
async def handle_xss_detection(
    ...
    try:
        ...
        try:
            security_event = SecurityEvent(
                user_id=None, type="XSS_ATTEMPT_DETECTED", ip=client_ip, ua=user_agent
            )
            db.add(security_event)
            db.commit()
...
def _analyze_csp_violation(
    csp_report: CSPViolationReport, client_ip: str, user_agent: str
) -> Dict[str, Any]:
