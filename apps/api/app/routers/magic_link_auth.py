    ...
    **Başarılı kullanım:**
    ...
    try:
        ...
        return MagicLinkConsumeResponse(
            ...
            if hasattr(result.user.role, "value")
            else str(result.user.role),
            session_id=result.session_id,
        ...
        logger.error(
            "Unexpected magic link consumption error",
            exc_info=True,
            extra={
                "token_prefix": consume_data.token[:8] if len(consume_data.token) >= 8 else "***",
                "ip_address": client_info["ip_address"],
        ...
        return create_magic_link_error_response(
            MagicLinkError("ERR-ML-SYSTEM-ERROR", "Magic link sistem hatası, lütfen tekrar deneyin")
        )
...
async def magic_link_health_check(db: Session = Depends(get_db)) -> Dict[str, Any]:
