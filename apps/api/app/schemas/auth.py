...
class UserRegisterRequest(BaseModel):
    ...
    email: EmailStr = Field(
        ..., description="Kullanıcı e-posta adresi", example="kullanici@example.com"
    )
    password: str = Field(
        ...,
        min_length=12,
    ...
    data_processing_consent: bool = Field(True, description="KVKK veri işleme rızası (zorunlu)")
    marketing_consent: bool = Field(False, description="Pazarlama iletişimi rızası (isteğe bağlı)")
...
class UserLoginRequest(BaseModel):
    ...
    password: str = Field(
        ..., min_length=1, max_length=128, description="Kullanıcı şifresi", example="GüçlüŞifre123!"
    )
    device_fingerprint: Optional[str] = Field(
        None, max_length=512, description="Cihaz parmak izi (güvenlik için)", example="fp_123abc..."
    )
...
class UserLoginResponse(BaseModel):
    ...
    mfa_required: Optional[bool] = Field(None, description="İki faktörlü doğrulama gerekli mi?")
...
class PasswordStrengthResponse(BaseModel):
    ...
    score: int = Field(..., ge=0, le=100, description="Şifre gücü skoru (0-100)")
    ok: bool = Field(..., description="Şifre minimum gereksinimleri karşılıyor mu?")
...
class PasswordForgotRequest(BaseModel):
    ...
    email: EmailStr = Field(
        ..., description="Şifre sıfırlanacak e-posta adresi", example="kullanici@example.com"
    )
...
class PasswordResetResponse(BaseModel):
    ...
    message: str = Field(default="Şifre başarıyla güncellendi.", description="Başarı mesajı")
    user_id: int = Field(..., description="Kullanıcı ID'si")
...
class AuthErrorResponse(BaseModel):
    ...
    error_code: str = Field(..., description="Hata kodu", example="ERR-AUTH-INVALID-CREDS")
    message: str = Field(..., description="Hata mesajı", example="E-posta adresi veya şifre hatalı")
...
class LogoutResponse(BaseModel):
    ...
    message: str = Field(default="Oturum başarıyla kapatıldı", description="Başarı mesajı")


class LogoutAllResponse(BaseModel):
    """Logout all sessions response schema."""

    message: str = Field(default="Tüm oturumlar başarıyla kapatıldı", description="Başarı mesajı")
