...
class MFASetupStartResponse(BaseModel):
    ...
    secret_masked: str = Field(
        ...,
        description="Maskelenmiş TOTP secret (güvenlik için)",
        example="ABCD****XYZE"
    )
...
class MFASetupVerifyResponse(BaseModel):
    ...
    message: str = Field(
        ...
        example="MFA başarıyla etkinleştirildi"
    )
...
class MFADisableResponse(BaseModel):
    ...
    message: str = Field(
        ...,
        description="Başarı mesajı",
        example="MFA başarıyla devre dışı bırakıldı"
    )
...
class MFAChallengeResponse(BaseModel):
    ...
    message: str = Field(
        default="MFA doğrulaması başarılı",
        description="Başarı mesajı"
    )
...
class MFABackupCodesResponse(BaseModel):
    ...
    codes_count: int = Field(
        ...,
        description="Oluşturulan kod sayısı",
        example=10
    )
...
class MFAStatusResponse(BaseModel):
    ...
    mfa_enabled_at: Optional[datetime] = Field(
        None, 
        description="MFA etkinleştirilme tarihi"
    )
    backup_codes_count: int = Field(
        ..., 
        description="Kalan yedek kod sayısı"
    )
    can_disable_mfa: bool = Field(
        ...,
        description="MFA devre dışı bırakılabilir mi?"
    )
    requires_mfa: bool = Field(
        ...,
        description="Bu kullanıcı için MFA zorunlu mu?"
    )
...
class MFAErrorResponse(BaseModel):
    ...
    error_code: str = Field(
        ...
        example="ERR-MFA-INVALID"
    )
    message: str = Field(
        ...,
        description="Hata mesajı",
        example="MFA kodu geçersiz"
    )
