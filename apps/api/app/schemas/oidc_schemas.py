...
class OIDCAuthStartResponse(BaseModel):
    ...
    state: str = Field(
        ..., description="OAuth2 state parameter for CSRF protection", min_length=32, max_length=256
    )

    expires_in: int = Field(
        default=900, description="State expiration time in seconds (15 minutes)", ge=300, le=1800
    )

    message: str = Field(
        default="OIDC kimlik doğrulama başlatıldı", description="Turkish success message"
    )
...
class OIDCCallbackRequest(BaseModel):
    ...
    code: str = Field(
        ..., description="Authorization code from Google", min_length=10, max_length=2048
    )

    state: str = Field(
        ..., description="OAuth2 state parameter for validation", min_length=32, max_length=256
    )
...
class OIDCUserProfile(BaseModel):
    ...
    sub: str = Field(..., description="OIDC subject identifier", min_length=1, max_length=255)

    email: EmailStr = Field(..., description="Email address from OIDC provider")

    email_verified: bool = Field(default=False, description="Whether email is verified by provider")

    name: Optional[str] = Field(None, description="Full name from OIDC provider", max_length=255)

    picture: Optional[str] = Field(None, description="Profile picture URL", max_length=500)

    locale: Optional[str] = Field(None, description="User locale from provider", max_length=10)


class OIDCAuthResponse(BaseModel):
    """Response schema for successful OIDC authentication."""

    access_token: str = Field(..., description="JWT access token for API requests", min_length=10)

    token_type: str = Field(default="bearer", description="Token type (always bearer)")

    expires_in: int = Field(
        ..., description="Access token expiration time in seconds", ge=300, le=86400
    )

    user_id: int = Field(..., description="Local user ID", ge=1)

    email: EmailStr = Field(..., description="User email address")

    full_name: Optional[str] = Field(None, description="User full name", max_length=255)

    role: str = Field(..., description="User role", pattern="^(admin|engineer|operator|viewer)$")

    is_new_user: bool = Field(
        default=False, description="Whether this is a newly created user account"
    )

    is_new_oidc_link: bool = Field(
        default=False, description="Whether this is a new OIDC account linkage"
    )

    profile: OIDCUserProfile = Field(..., description="OIDC profile information")

    message: str = Field(default="OIDC girişi başarılı", description="Turkish success message")
...
class OIDCErrorResponse(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(
        None, description="Additional error details (sanitized)"
    )
...
class OIDCAccountInfo(BaseModel):
    ...
    provider: str = Field(..., description="OIDC provider name", example="google")

    email: EmailStr = Field(..., description="Email from OIDC provider")

    email_verified: bool = Field(..., description="Email verification status")

    picture: Optional[str] = Field(None, description="Profile picture URL")

    first_login_at: Optional[datetime] = Field(None, description="First login timestamp")
...
class OIDCStatusResponse(BaseModel):
    ...
    google_oauth_enabled: bool = Field(..., description="Whether Google OAuth is enabled")
    ...
    scopes: list[str] = Field(
        default=["openid", "email", "profile"], description="OAuth scopes requested"
    )

    redirect_uri: Optional[str] = Field(None, description="OAuth redirect URI for this environment")
...
def create_oidc_error_response(
    error_code: str, details: Optional[Dict[str, Any]] = None
) -> OIDCErrorResponse:
    """
    ...
    return OIDCErrorResponse(error_code=error_code, message=message, details=details)
