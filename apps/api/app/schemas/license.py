...
class LicenseAssignRequest(BaseModel):
    ...
    user_id: Optional[UUID] = Field(
        None, description="Target user ID (admin only, auto-filled for self-assignment)"
    )
    type: str = Field(..., description="License duration type", regex="^(3m|6m|12m)$")
    scope: Dict[str, Any] = Field(..., description="License scope configuration (features, limits)")
    starts_at: Optional[datetime] = Field(
        None, description="License start time (RFC3339, defaults to now())"
    )
...
class LicenseExtendRequest(BaseModel):
    ...
    user_id: Optional[UUID] = Field(
        None, description="Target user ID (admin only, auto-filled for self-service)"
    )
    license_id: Optional[UUID] = Field(
        None, description="Specific license ID to extend (optional, uses active if not specified)"
    )
    type: str = Field(..., description="Extension duration type", regex="^(3m|6m|12m)$")
...
class LicenseCancelRequest(BaseModel):
    ...
    license_id: Optional[UUID] = Field(
        None, description="Specific license ID to cancel (optional, uses active if not specified)"
    )
...
class LicenseErrorCodes:
    ...
    ERROR_MESSAGES = {
        ...
        NOT_FOUND: {"en": "License not found", "tr": "Lisans bulunamadı"},
        ALREADY_CANCELED: {"en": "License is already canceled", "tr": "Lisans zaten iptal edilmiş"},
        VALIDATION_ERROR: {"en": "Request validation failed", "tr": "İstek doğrulama başarısız"},
        DATABASE_ERROR: {"en": "Database operation failed", "tr": "Veritabanı işlemi başarısız"},
        INTERNAL_ERROR: {"en": "Internal server error", "tr": "Sunucu iç hatası"},
    }

    @classmethod
    def get_error_response(
        cls, code: str, details: Optional[Dict[str, Any]] = None
    ) -> LicenseErrorResponse:
        """Create standardized error response."""
        messages = cls.ERROR_MESSAGES.get(code, {"en": "Unknown error", "tr": "Bilinmeyen hata"})

        return LicenseErrorResponse(
            code=code, message=messages["en"], message_tr=messages["tr"], details=details
        )
