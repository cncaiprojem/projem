    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "details": {"required_role": "admin", "user_role": "engineer"},
                "timestamp": "2024-01-15T10:30:00Z",
            }
        }
    )

    error_code: RBACErrorCode = Field(..., description="Specific RBAC error code")
    message: str = Field(
        ..., description="Turkish localized error message", min_length=1, max_length=500
    )
    details: Optional[Dict[str, Any]] = Field(None, description="Additional error details")
    timestamp: datetime = Field(..., description="Error occurrence timestamp (UTC)")
...
class PermissionScope(BaseModel):
    ...
    description: str = Field(
        ..., description="Human-readable scope description", min_length=1, max_length=200
    )
    resource: str = Field(
        ..., description="Resource this scope applies to", min_length=1, max_length=50
    )
    action: str = Field(..., description="Action this scope allows", min_length=1, max_length=50)
...
class RolePermissions(BaseModel):
    ...
    scopes: Set[str] = Field(..., description="Set of permission scopes for this role")
    hierarchy_level: int = Field(
        ..., description="Role hierarchy level (higher = more permissions)", ge=1, le=10
    )
...
class UserPermissionSummary(BaseModel):
    ...
    scopes: List[str] = Field(..., description="List of permission scopes user has")
    is_admin: bool = Field(..., description="Whether user has admin privileges")
    is_active: bool = Field(..., description="Whether user account is active")
    last_permission_check: Optional[datetime] = Field(
        None, description="Last time permissions were checked"
    )
...
class SecurityEventRequest(BaseModel):
    ...
    event_type: str = Field(..., description="Type of security event", min_length=1, max_length=100)
    user_id: Optional[int] = Field(None, description="Associated user ID", gt=0)
    ip_address: Optional[str] = Field(None, description="Source IP address", max_length=45)
    user_agent: Optional[str] = Field(None, description="User agent string", max_length=1000)
...
class SecurityEventResponse(BaseModel):
    ...
    id: int = Field(..., description="Security event ID", gt=0)
...
class PermissionCheckRequest(BaseModel):
    ...
    user_id: int = Field(..., description="User ID to check permissions for", gt=0)
    resource: str = Field(
        ..., description="Resource to check access for", min_length=1, max_length=100
    )
    action: str = Field(
        ..., description="Action to check permission for", min_length=1, max_length=50
    )
    context: Optional[Dict[str, Any]] = Field(
        None, description="Additional context for permission check"
    )
...
class PermissionCheckResponse(BaseModel):
    ...
    user_id: int = Field(..., description="User ID that was checked")
    ...
    user_role: UserRole = Field(..., description="User's current role")
    required_scope: Optional[str] = Field(None, description="Required scope for this permission")
    check_timestamp: datetime = Field(..., description="When permission check was performed")
...
class RoleUpdateRequest(BaseModel):
    ...
    user_id: int = Field(..., description="User ID to update role for", gt=0)
    new_role: UserRole = Field(..., description="New role to assign")
    reason: str = Field(..., description="Reason for role change", min_length=10, max_length=500)
    effective_date: Optional[datetime] = Field(
        None, description="When role change should take effect (default: immediately)"
    )
...
class RoleUpdateResponse(BaseModel):
    ...
    old_role: UserRole = Field(..., description="Previous user role")
    new_role: UserRole = Field(..., description="New user role")
    updated_by: int = Field(..., description="Admin user ID who made the change")
    updated_at: datetime = Field(..., description="When role was updated")
...
class SystemPermissionsResponse(BaseModel):
    ...
    role_hierarchy: Dict[str, int] = Field(..., description="Role hierarchy levels")
    total_users_by_role: Dict[str, int] = Field(..., description="Count of users by role")
    last_updated: datetime = Field(..., description="Last time permission system was updated")
...
class ScopeValidator:
    ...
    VALID_ACTIONS = {"read", "write", "create", "delete", "upload", "users", "system", "billing"}
    ...
    def is_valid_scope(cls, scope: str) -> bool:
        ...
        return resource.lower() in cls.VALID_RESOURCES and action.lower() in cls.VALID_ACTIONS
