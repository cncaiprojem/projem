...
def validate_sms_constraint_fix() -> Tuple[bool, str]:
    ...
    migration_path = Path("apps/api/alembic/versions/20250819_0000-task_47_notification_service_email_sms_provider_fallback.py")
    ...
    return len(issues) == 0, "; ".join(issues) if issues else "SMS constraint correctly set to = 160"
...
def validate_no_security_event_import() -> Tuple[bool, str]:
    ...
    return len(issues) == 0, "; ".join(issues) if issues else "No SecurityEventService imports or code references found"
...
def validate_unique_constraint_as_index() -> Tuple[bool, str]:
    ...
    if re.search(r"sa\.UniqueConstraint\([^)]*'type'[^)]*'channel'[^)]*'language'[^)]*'is_active'", content):
        issues.append("Still using UniqueConstraint instead of Index")
    
    # Check for correct Index with partial where clause
    if not re.search(r"sa\.Index\([^)]*'uq_notification_templates_active'[^)]*unique=True[^)]*postgresql_where", content):
...
def validate_no_sql_injection() -> Tuple[bool, str]:
    ...
    insert_section = content[content.find("def insert_default_templates"):content.find("def downgrade")]
...
def validate_session_management() -> Tuple[bool, str]:
    ...
    method_section = content[content.find("def get_active_template"):content.find("def create_license_reminder_templates")]
    ...
    return len(issues) == 0, "; ".join(issues) if issues else "Properly accepts session as parameter"
