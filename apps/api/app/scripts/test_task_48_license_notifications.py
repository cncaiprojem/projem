...
def setup_test_database():
    ...
    engine = create_engine(
        "postgresql+psycopg2://freecad:freecad@localhost:5432/freecad",
        echo=False
    )
...
def test_license_query(session):
    ...
    for days_out in [7, 3, 1]:
        ...
        for license_obj in licenses:
            ...
            print(f"    - License {license_obj.id}: {license_obj.type} (actual days: {remaining_days})")
...
def test_notification_creation(session):
    ...
    for days_out in [7, 3, 1]:
        ...
        print(f"    D-{days_out}: {licenses_count} licenses → {queued_count} queued, {skipped_count} skipped")
...
def test_duplicate_prevention(session):
    ...
    print(f"  🎯 Duplicate prevention: {total_skipped} duplicates skipped, {total_queued} new notifications")
...
def main():
    ...
    try:
        ...
        session.query(NotificationDelivery).delete()
        session.query(License).filter(License.user_id.in_(
