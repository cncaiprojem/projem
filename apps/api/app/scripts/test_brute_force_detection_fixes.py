        ...
        **Fixed Gemini Feedback Issues**:
        ...
        if account_key in self.failed_attempts:
            self.failed_attempts[account_key] = [
                attempt
                for attempt in self.failed_attempts[account_key]
...
async def test_ipv4_brute_force_detection():
    ...
    ipv4_addresses = ["192.168.1.100", "10.0.0.50", "172.16.1.200", "203.0.113.45"]
    ...
    turkish_scenarios = [
        {
            ...
            "ip": "85.104.23.156",
            "description": "Turkish name with special chars",
        },
        {
            "username": "zeynep@türkiye.com",
            "ip": "213.74.194.22",
            "description": "Turkish email domain",
        },
        {
            "username": "müdür.istanbul",
            "ip": "78.186.145.89",
            "description": "Turkish professional title",
        },
        {
            "username": "şirket_admin",
            "ip": "195.142.76.134",
            "description": "Turkish company admin",
        },
    ]
...
async def generate_comprehensive_test_report(all_results: List[BruteForceTestResult]):
    ...
    for test_name, results in test_groups.items():
        ...
        for result in results:
            ...
            print(
                f"{status} | IP: {result.masked_ip} | User: {result.username} | Attempts: {result.attempts}"
            )
    ...
    # Save report to file
    report_filename = (
        f"/tmp/brute_force_test_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
    )
