...
# Configure logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(levelname)s - [RATE-LIMIT-TEST] %(message)s"
)
...
class UltraEnterpriseRateLimiter:
    ...
    **Security Features**:
    ...
    async def is_rate_limited(
        ...
        self.request_counts[key] = [
            req_time for req_time in self.request_counts[key] if req_time > window_start
        ]
...
async def test_endpoint_rate_limiting():
    ...
    for scenario in test_scenarios:
        ...
        print(
            f"IP: {scenario['client_ip']} → {rate_limiter.mask_ip_address(scenario['client_ip'])}"
        )
        ...
        for request_num in range(scenario["max_requests"] + 5):  # Try 5 extra
            ...
            is_limited, rate_info = await rate_limiter.is_rate_limited(
                scenario["endpoint"], scenario["client_ip"], scenario["user_id"]
            )
            ...
            if is_limited:
                print(
                    f"🚫 Request #{request_num + 1}: Rate limited (429) - {rate_info['current_count']}/{rate_info['limit']}"
                )
                break
            else:
                print(
                    f"✅ Request #{request_num + 1}: Allowed (200) - {rate_info['remaining']} remaining"
                )
...
async def test_brute_force_integration():
    ...
    for scenario in attack_scenarios:
        ...
        print(
            f"Attacker IP: {scenario['attacker_ip']} → {rate_limiter.mask_ip_address(scenario['attacker_ip'])}"
        )
        ...
        for attack_attempt in range(20):  # Try 20 rapid requests
            ...
            is_limited, rate_info = await rate_limiter.is_rate_limited(
                scenario["target_endpoint"], scenario["attacker_ip"]
            )
            ...
            if is_limited:
                ...
                response_status = 429
                print(
                    f"🛡️  Attack attempt #{attack_attempt + 1}: BLOCKED ({consecutive_blocked} consecutive blocks)"
                )
            else:
                consecutive_blocked = 0
                response_status = 200
                print(
                    f"⚠️  Attack attempt #{attack_attempt + 1}: Allowed - {rate_info['remaining']} remaining"
                )
        ...
        print(
            f"✅ Brute force simulation completed - {consecutive_blocked} final consecutive blocks"
        )
...
async def test_legitimate_user_protection():
    ...
    for i in range(10):  # Test 10 legitimate requests
        ...
        test_result = RateLimitTestResult(
            ...
            response_time_ms=response_time,
            rate_limit_headers={"X-RateLimit-Remaining": str(rate_info["remaining"])},
            timestamp=datetime.utcnow(),
...
async def test_turkish_kvkv_rate_limit_logging():
    ...
    for scenario in turkish_scenarios:
        ...
        for request_num in range(5):
            ...
            masked_ip = rate_info["masked_ip"]
            masked_user_id = (
                f"{scenario['user_id'][:3]}***@{scenario['user_id'].split('@')[1][:3]}***"
                if "@" in scenario["user_id"]
                else f"{scenario['user_id'][:3]}***"
            )
            ...
            print(
                f"✅ Request #{request_num + 1}: PII masked - IP: {masked_ip}, User: {masked_user_id}"
            )
...
async def generate_rate_limit_test_report(all_results: List[RateLimitTestResult]):
    ...
    for test_name, results in test_groups.items():
        ...
        for result in results:
            ...
            print(
                f"  {status} | {rate_status} | {result.endpoint} | {result.masked_ip} | {result.response_time_ms:.1f}ms"
            )
...
async def main():
    ...
    try:
        ...
        # Save report
        report_filename = (
            f"/tmp/rate_limiting_test_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        # Final validation
        success_rate = (
            (report["successful_tests"] / report["total_tests"]) * 100
