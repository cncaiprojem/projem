...
class CSRFProtectionDemo:
    ...
    def demo_csrf_protection_flow(self):
        ...
        self.print_result(
            "Geçerli CSRF İsteği",
            success,
            "CSRF token eşleşiyor, istek geçti" if success else "İstek beklenmedik şekilde engellendi"
        )
        ...
        self.print_result(
            ...
            "CSRF token eksik, istek engellendi" if not success else "İstek beklenmedik şekilde geçti"
        )
        
        # Test 4: Invalid CSRF token
        self.print_section("Test 4: Geçersiz CSRF Token")
        success = self.test_csrf_protected_request(
            csrf_token="invalid_token_123456789",
            use_browser_headers=True,
            with_auth=True
        )
        self.print_result(
            "Geçersiz CSRF Token",
            not success,  # Should fail
            "CSRF token eşleşmiyor, istek engellendi" if not success else "İstek beklenmedik şekilde geçti"
        )
        ...
        self.print_result(
            "API Client İsteği",
            success,
            "API client için CSRF atlandı" if success else "API client isteği beklenmedik şekilde engellendi"
        )
        ...
        try:
            ...
            self.print_result(
                "GET İsteği",
                success,
                "GET isteği için CSRF atlandı" if success else "GET isteği beklenmedik şekilde engellendi"
            )
