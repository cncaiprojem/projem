...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - [ASYNC-REDIS-TEST] %(message)s'
)
...
class UltraEnterpriseRedisManager:
    ...
    async def set_with_expiry(self, key: str, value: Any, ttl_seconds: int = 3600, namespace: str = "cache") -> bool:
    ...
    async def increment_counter(self, key: str, namespace: str = "counters", increment: int = 1, ttl_seconds: int = 3600) -> Optional[int]:
...
async def test_basic_redis_operations():
    ...
    test_results.append(RedisTestResult(
    ...
    for api_key in api_keys:
        ...
        for request_num in range(max_requests + 2):  # Try 2 extra requests
            ...
            if rate_limit_exceeded:
                print(f"🚫 Request #{request_num + 1}: Rate limit exceeded ({current_count} requests)")
                break
            else:
...
async def generate_redis_test_report(all_results: List[RedisTestResult]):
    ...
    for test_name, results in test_groups.items():
        ...
        for result in results:
            ...
            print(f"{status} | {result.operation} | {result.key} | {result.execution_time_ms:.2f}ms")
