...
def setup_database_logging(
    engine: Engine, log_queries: bool = True, slow_query_threshold_ms: int = 1000
) -> None:
    """
    ...
    def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
        ...
        if log_queries or (total_time and total_time > slow_query_threshold_ms):
            log_database_query(
                query=statement,
                params=parameters
                if len(str(parameters)) < 1000
                else None,  # Don't log huge param lists
    ...
    def handle_error(exception_context):
        ...
        logger.error(
            ...
            if exception_context.parameters and len(str(exception_context.parameters)) < 1000
            else None,
            exc_info=True,
        )
