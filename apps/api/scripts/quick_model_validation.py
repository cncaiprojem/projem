...
def validate_models():
    ...
    expected_tables = {
        'users', 'sessions', 'licenses', 'models', 'jobs', 'cam_runs',
        'sim_runs', 'artefacts', 'notifications', 'erp_mes_sync',
        'invoices', 'payments', 'audit_logs', 'security_events',
    ...
    if 'users' in models:
        ...
        found_fields = [field for field in turkish_fields if any(col.name == field for col in users_table.columns)]
    ...
    print(f"+ ERD compliance: {'Perfect' if not missing_tables and not extra_tables else 'Needs attention'}")
