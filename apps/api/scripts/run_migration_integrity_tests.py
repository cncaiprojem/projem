...
class MigrationTestRunner:
    ...
    def run_migration_safety_tests(self) -> bool:
        ...
        if self.generate_report:
            test_args.extend([
                "--junitxml", str(self.report_dir / "migration_safety_report.xml"),
                "--cov=app",
    ...
    def run_constraint_validation_tests(self) -> bool:
        ...
        test_args = [
            str(self.test_dir / "integration" / "test_migration_integrity.py::TestDatabaseConstraints"),
            "-v" if self.verbose else "-q",
            "--tb=short",
            "-m", "constraint_validation"
        ]
        
        if self.generate_report:
            test_args.extend([
                "--junitxml", str(self.report_dir / "constraint_validation_report.xml"),
                "--cov=app.models",
    ...
    def run_audit_integrity_tests(self) -> bool:
        ...
        test_args = [
            str(self.test_dir / "integration" / "test_migration_integrity.py::TestAuditChainIntegrity"),
            "-v" if self.verbose else "-q",
            "--tb=short",
            "-m", "audit_integrity"
        ]
        ...
        if self.generate_report:
            test_args.extend([
                "--junitxml", str(self.report_dir / "audit_integrity_report.xml"),
    ...
    def run_performance_tests(self) -> bool:
        ...
        test_args = [
            str(self.test_dir / "integration" / "test_migration_integrity.py::TestQueryPerformance"),
            "-v" if self.verbose else "-q",
            "--tb=short",
            "-m", "performance_validation"
        ]
    ...
    def run_integration_tests(self) -> bool:
        ...
        test_args = [
            str(self.test_dir / "integration" / "test_migration_integrity.py::TestMigrationIntegrityIntegration"),
            "-v" if self.verbose else "-q",
            "--tb=short"
        ]
        ...
        if self.generate_report:
            test_args.extend([
                "--junitxml", str(self.report_dir / "integration_tests_report.xml"),
                "--cov=app",
    ...
    def run_turkish_compliance_tests(self) -> bool:
        ...
        test_args = [
            ...
            "-v" if self.verbose else "-q",
            "--tb=short",
            "-m", "turkish_compliance"
        ]
        
        if self.generate_report:
            test_args.extend([
                "--junitxml", str(self.report_dir / "turkish_compliance_report.xml")
            ])
    ...
    def run_safety_checks(self) -> bool:
        ...
        try:
            ...
            is_safe, warnings = checker.validate_migration_safety(engine, "migration_integrity_tests")
    ...
    def generate_summary_report(self):
        ...
        report = {
            ...
            "summary": {
                ...
                "failed_phases": sum(1 for r in self.results.values() if not r.get("success", True)),
                "overall_success": all(r.get("success", False) for r in self.results.values())
            }
        }
        
        # Save JSON report
        report_file = self.report_dir / f"migration_integrity_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        print(f"✅ Passed Phases: {report['summary']['passed_phases']}/{report['summary']['total_phases']}")
        print(f"❌ Failed Phases: {report['summary']['failed_phases']}/{report['summary']['total_phases']}")
        print()
...
Examples:
  ...
  python scripts/run_migration_integrity_tests.py --compliance --performance
    ...
    parser.add_argument(
        "--compliance", "-c",
        action="store_true",
        help="Run Turkish compliance-specific tests"
    )
    parser.add_argument(
        "--performance", "-p",
        action="store_true",
