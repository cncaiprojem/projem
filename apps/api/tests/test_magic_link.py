...
from app.schemas.magic_link_schemas import (
    MagicLinkRequestRequest, MagicLinkConsumeRequest
)
...
class TestMagicLinkService:
    ...
    def test_request_magic_link_existing_user(self, db: Session, test_user: User):
        ...
        with patch.object(magic_link_service, '_send_magic_link_email') as mock_email:
            ...
            magic_link = db.query(MagicLink).filter(
                MagicLink.email == test_user.email
            ).first()
    ...
    def test_request_magic_link_nonexistent_user(self, db: Session):
        ...
        with patch.object(magic_link_service, '_send_magic_link_email') as mock_email:
            ...
            magic_link = db.query(MagicLink).filter(
                MagicLink.email == "nonexistent@example.com"
            ).first()
...
class TestMagicLinkSecurity:
    ...
    def test_audit_logging(self, db: Session, test_user: User):
        ...
        audit_log = db.query(AuditLog).filter(
            AuditLog.action == "magic_link_requested"
        ).first()
    ...
    def test_device_fingerprint_tracking(self, db: Session, test_user: User):
        ...
        magic_link = MagicLink(
            email=test_user.email,
            nonce="test-fingerprint",
            device_fingerprint="fp_original123"
        )
        db.add(magic_link)
        db.commit()
        ...
        magic_link.consume(
            device_fingerprint="fp_different456"
        )
