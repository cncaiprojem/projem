...
class TestPDFService:
    ...
    def pdf_service(self, mock_s3_service, mock_audit_service):
        ...
        return PDFService(s3_service=mock_s3_service, audit_service=mock_audit_service)

    async def test_generate_pdf_with_reportlab_fallback(
        self, pdf_service, sample_invoice, mock_s3_service, mock_audit_service
    ):
        ...
        with (
            ...
            pdf_url, checksum = await pdf_service.generate_invoice_pdf(
                db=mock_db, invoice=sample_invoice
            )
    ...
    async def test_generate_invoice_pdf_success_weasyprint(
        ...
        # Mock WeasyPrint
        with patch("weasyprint.HTML") as mock_html, patch("weasyprint.CSS") as mock_css:
    ...
    async def test_generate_invoice_pdf_fallback_to_reportlab(
        ...
        with (
            patch("weasyprint.HTML", side_effect=Exception("WeasyPrint failed")),
    ...
    async def test_generate_invoice_pdf_both_renderers_fail(self, pdf_service, sample_invoice):
        ...
        with (
            patch("weasyprint.HTML", side_effect=Exception("WeasyPrint failed")),
            patch.object(
                pdf_service, "_generate_with_reportlab", side_effect=Exception("ReportLab failed")
            ),
        ):
            # Test PDF generation failure
            with pytest.raises(PDFGenerationError) as exc_info:
                await pdf_service.generate_invoice_pdf(db=mock_db, invoice=sample_invoice)
    ...
    async def test_generate_invoice_pdf_existing_pdf_no_force(self, pdf_service, sample_invoice):
        ...
        # Test PDF generation
        pdf_url, checksum = await pdf_service.generate_invoice_pdf(
            db=mock_db, invoice=sample_invoice, force_regenerate=False
        )
    ...
    async def test_get_invoice_pdf_url_success(self, pdf_service, sample_invoice, mock_s3_service):
        ...
        assert pdf_url == "https://example.com/presigned-url"
        mock_s3_service.get_presigned_url.assert_called_once_with(
            bucket_name="invoices", object_key="invoices/2024/08/test.pdf", expires_in=120
        )
    ...
    async def test_get_invoice_pdf_url_s3_error(self, pdf_service, sample_invoice, mock_s3_service):
    ...
    async def test_upload_pdf_to_storage_metadata(self, pdf_service, mock_s3_service):
        ...
        mock_s3_service.set_object_legal_hold.assert_called_once_with(
            bucket_name="invoices", object_key=object_key, legal_hold=True
        )

    async def test_upload_pdf_to_storage_s3_error(self, pdf_service, mock_s3_service):
        """Test error handling during S3 upload."""
        mock_s3_service.upload_file_content.side_effect = Exception("S3 upload failed")

        with pytest.raises(PDFGenerationError) as exc_info:
            await pdf_service._upload_pdf_to_storage(
                pdf_content=b"test", object_key="test.pdf", invoice_number="test", checksum="test"
            )
...
class TestInvoicePDFIntegration:
    ...
    def test_pdf_path_generation(self):
        ...
        actual_path = (
            f"invoices/{invoice_date.year:04d}/{invoice_date.month:02d}/{invoice_number}.pdf"
        )
    ...
    def test_turkish_content_in_pdf_template(self):
        ...
        pdf_service = PDFService(
            s3_service=Mock(spec=S3Service), audit_service=Mock(spec=AuditService)
        )
