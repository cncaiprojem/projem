...
class TestJWTService:
    ...
    def test_create_access_token_success(self, db: DBSession, test_user: User, test_session: Session):
    ...
    def test_create_access_token_with_custom_scopes(self, db: DBSession, test_user: User, test_session: Session):
    ...
    def test_verify_access_token_success(self, db: DBSession, test_user: User, test_session: Session):
    ...
    def test_verify_revoked_session_token(self, db: DBSession, test_user: User, test_session: Session):
...
class TestJWTAPIEndpoints:
    ...
    def test_refresh_token_endpoint_success(self, client: TestClient, db: DBSession, test_user: User):
    ...
    def test_list_active_sessions_endpoint(self, client: TestClient, db: DBSession, test_user: User):
        ...
        current_session_marked = any(
            session['is_current'] for session in data['sessions']
        )
...
class TestJWTSecurityFeatures:
    ...
    def test_jwt_token_expiration_timing(self, db: DBSession, test_user: User, test_session: Session):
    ...
    def test_audit_logging_completeness(self, db: DBSession, test_user: User):
        ...
        result2 = token_service.rotate_refresh_token(db=db, current_refresh_token=result1.refresh_token)
        ...
        audit_actions = db.query(AuditLog.action).filter(
            AuditLog.user_id == test_user.id
        ).all()
