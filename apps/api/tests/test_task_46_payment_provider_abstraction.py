...
class TestWebhookProcessing:
    ...
    def test_webhook_processing_success(self, db: Session, sample_payment):
        ...
        webhook_event = db.query(PaymentWebhookEvent).filter(
            PaymentWebhookEvent.event_id == "evt_test_123"
        ).first()
        ...
        audit_log = db.query(PaymentAuditLog).filter(
    ...
    def test_webhook_processing_idempotency(self, db: Session, sample_payment):
        ...
        webhook_events = db.query(PaymentWebhookEvent).filter(
            PaymentWebhookEvent.event_id == "evt_test_duplicate"
        ).all()
...
class TestPaymentAPI:
    ...
    def test_webhook_endpoint_rate_limited(self):
        ...
        with patch('apps.api.app.services.rate_limiting_service.RateLimitingService.check_rate_limit', return_value=False):
            response = client.post(
                "/payments/webhook",
                json={},
