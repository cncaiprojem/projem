...
class TestAuditSecurityIntegration:
    ...
    def test_complete_audit_chain_integrity(self):
        ...
        genesis_hash = AuditLog.compute_chain_hash(AuditLog.get_genesis_hash(), genesis_payload)
        ...
        user_create_payload = {
            ...
            "entity_id": 1,
            "action": "CREATE",
            "data": {"email": "admin@freecad.com", "role": "admin", "created_by": "system"},
        }
        user_create_hash = AuditLog.compute_chain_hash(genesis_hash, user_create_payload)
        ...
        job_create_payload = {
            ...
            "data": {
                "type": "CAD_GENERATE",
                "priority": "high",
                "params": {"model_type": "part", "material": "aluminum"},
            },
        }
        job_create_hash = AuditLog.compute_chain_hash(user_create_hash, job_create_payload)
        ...
        payment_hash = AuditLog.compute_chain_hash(job_create_hash, payment_payload)
    ...
    def test_security_event_patterns(self):
        ...
        access_denied = SecurityEvent.create_access_denied(
            user_id=123, ip="192.168.1.50", ua="FreeCAD-Client/1.0"
        )
        security_events.append(access_denied)

        # Suspicious activity detection
        suspicious = SecurityEvent.create_suspicious_activity(
            user_id=123, activity_type="multiple_locations", ip="10.0.0.200", ua="Mobile-App/2.0"
        )
        security_events.append(suspicious)

        # Verify event properties
        assert (
            len(security_events) == 8
        )  # 1 failed + 5 brute force + 1 access denied + 1 suspicious
    ...
    def test_audit_and_security_correlation(self):
        ...
        failed_login = SecurityEvent.create_login_failed(
            user_id=456, ip="198.51.100.50", ua="Automated-Bot/1.0"
        )
        ...
        login_audit_hash = AuditLog.compute_chain_hash(
            AuditLog.get_genesis_hash(), login_attempt_payload
        )
        ...
        lockout_audit_hash = AuditLog.compute_chain_hash(login_audit_hash, lockout_payload)
    ...
    def test_enterprise_compliance_patterns(self):
        ...
        consent_hash = AuditLog.compute_chain_hash(AuditLog.get_genesis_hash(), consent_payload)
        ...
        processing_hash = AuditLog.compute_chain_hash(consent_hash, processing_payload)
        ...
        export_hash = AuditLog.compute_chain_hash(processing_hash, export_payload)
        ...
        assert processing_audit.get_payload_field("data_categories") == [
            "design_parameters",
            "user_preferences",
        ]
    ...
    def test_financial_audit_precision(self):
        ...
        financial_payload = {
            "transaction": {
                ...
                "customer": {"id": 123, "tax_number": "1234567890", "type": "corporate"},
                "items": [
                    {
        ...
        financial_hash = AuditLog.compute_chain_hash(AuditLog.get_genesis_hash(), financial_payload)
        ...
        same_hash = AuditLog.compute_chain_hash(AuditLog.get_genesis_hash(), financial_payload)
    ...
    def test_unicode_and_turkish_support(self):
        ...
        turkish_hash = AuditLog.compute_chain_hash(AuditLog.get_genesis_hash(), turkish_payload)
        ...
        same_hash = AuditLog.compute_chain_hash(AuditLog.get_genesis_hash(), turkish_payload)
