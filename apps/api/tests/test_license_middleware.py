...
class TestLicenseGuardMiddleware:
    ...
    async def test_check_license_valid(
        self, mock_get_db, mock_get_license, middleware, mock_request, mock_license_active
    ):
    ...
    @patch("app.middleware.license_middleware.LicenseService.get_active_license")
    @patch("app.middleware.license_middleware.get_db")
    async def test_check_license_no_license(
        self, mock_get_db, mock_get_license, middleware, mock_request
    ):
    ...
    @patch("app.middleware.license_middleware.LicenseService.get_active_license")
    @patch("app.middleware.license_middleware.get_db")
    async def test_check_license_expired(
        self, mock_get_db, mock_get_license, middleware, mock_request, mock_license_expired
    ):
        ...
        with patch.object(
            middleware, "_revoke_user_sessions_on_expiry", return_value=True
        ) as mock_revoke:
            result = await middleware._check_license_and_enforce(
                mock_request, 123, "test-request-id"
            )
    ...
    async def test_revoke_user_sessions_on_expiry(
        self, mock_audit, mock_revoke_sessions, middleware, mock_request
    ):
    ...
    async def test_revoke_user_sessions_thread_safety(self, middleware, mock_request):
        ...
        with patch(
            "app.middleware.license_middleware.session_service.revoke_all_user_sessions",
            return_value=2,
        ):
            with patch(
                "app.middleware.license_middleware.audit_service.log_business_event",
    ...
    async def test_dispatch_valid_license(self, mock_get_user, middleware):
        ...
        with patch.object(
            middleware, "_check_license_and_enforce", return_value=None
        ) as mock_check:
            result = await middleware.dispatch(mock_request, mock_call_next)
    ...
    async def test_dispatch_invalid_license(self, mock_get_user, middleware):
        ...
        with patch.object(
            middleware, "_check_license_and_enforce", return_value=error_response
        ) as mock_check:
            result = await middleware.dispatch(mock_request, mock_call_next)
...
class TestLicenseMiddlewareEdgeCases:
    ...
    async def test_correlation_id_propagation(self):
        ...
        with patch(
            "app.middleware.license_middleware.get_current_user_from_request"
        ) as mock_get_user:
            mock_get_user.return_value = None
    ...
    async def test_license_check_with_database_error(self):
        ...
        with patch(
            "app.middleware.license_middleware.get_db_session_for_middleware"
        ) as mock_session:
            mock_db = Mock()
            ...
            with patch(
                "app.middleware.license_middleware.LicenseService.get_active_license"
            ) as mock_get_license:
                mock_get_license.side_effect = IntegrityError("Constraint violation", "", "")

                result = await middleware._check_license_and_enforce(
                    mock_request, 123, "test-req-id"
                )
    ...
    def test_cache_clearing_with_active_entries(self):
        ...
        from app.middleware.license_middleware import (
            _license_expiry_processed,
            _license_expiry_lock,
        )
