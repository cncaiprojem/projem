...
class TestProductionHardeningMiddleware:
    ...
    def test_https_redirect_in_production(self, client):
        ...
        with patch(
            "app.middleware.dev_mode_middleware.ProductionHardeningMiddleware._is_http_request",
            return_value=True,
        ):
            response = client.get("/test")
    ...
    def test_no_production_hardening_in_dev(self, client):
        ...
        assert (
            "X-Security-Level" not in response.headers
            or response.headers.get("X-Security-Level") != "ultra-enterprise-banking"
        )
...
class TestEnvironmentValidationMiddleware:
    ...
    @patch("app.core.environment.environment.ENV", EnvironmentMode.PRODUCTION)
    @patch(
        "app.core.environment.environment.SECRET_KEY",
...
class TestEnvironmentService:
    ...
    def test_is_feature_enabled(self):
        ...
        assert (
            environment_service.is_feature_enabled("audit_logging") == environment.AUDIT_LOG_ENABLED
        )
