...
class TestFinancialPrecision:
    ...
    def test_complex_tax_scenarios(self):
        ...
        expected_tax = int(
            (Decimal("30000") * Decimal("33.33") / Decimal("100")).to_integral_value(ROUND_HALF_UP)
        )
    ...
    def test_enterprise_tax_methods(self):
        ...
        expected_subtotal = int(
            (Decimal("12000") / Decimal("1.20")).to_integral_value(ROUND_HALF_UP)
        )
        ...
        # 12000 * 20 / 100 = 2400
        expected_tax_exclusive = int(
            (Decimal("12000") * Decimal("20") / Decimal("100")).to_integral_value(ROUND_HALF_UP)
        )
