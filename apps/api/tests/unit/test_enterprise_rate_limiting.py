...
class TestEnterpriseRateLimitingService:
    ...
    def mock_redis_client(self):
        ...
        mock_redis = Mock()
        mock_redis.pipeline.return_value.__enter__.return_value.execute.return_value = [None, 1, None, None]
        return mock_redis
    ...
    async def test_check_rate_limit_allowed(self, rate_service, mock_request, mock_user, mock_db):
        ...
        rate_service.redis_client.pipeline.return_value.__enter__.return_value.execute.return_value = [None, 2, None, None]
    ...
    async def test_check_rate_limit_exceeded(self, rate_service, mock_request, mock_user, mock_db):
        ...
        rate_service.redis_client.pipeline.return_value.__enter__.return_value.execute.return_value = [None, 5, None, None]
...
class TestRateLimitDependency:
    ...
    @pytest.mark.asyncio
    async def test_dependency_allowed_request(self, rate_limit_dependency, mock_request, mock_db, mock_rate_service):
        ...
        result = await rate_limit_dependency(
            ...
            mock_db,
            current_user=None,
            rate_service=mock_rate_service
        )
        assert result is None  # No exception raised
    
    @pytest.mark.asyncio
    async def test_dependency_blocked_request(self, rate_limit_dependency, mock_request, mock_db, mock_rate_service):
    ...
    @pytest.mark.asyncio
    async def test_dependency_service_failure(self, rate_limit_dependency, mock_request, mock_db, mock_rate_service):
