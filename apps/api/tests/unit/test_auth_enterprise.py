...
from app.schemas.auth import (
    UserRegisterRequest, UserLoginRequest, PasswordStrengthRequest,
    PasswordForgotRequest, PasswordResetRequest
)
...
class TestPasswordService:
    ...
    def test_password_strength_validation_repeated_patterns(self):
        ...
        for password in pattern_passwords:
            result = password_service.validate_password_strength(password)
            assert result.ok is False
            assert any("tekrarlayan" in feedback.lower() or "desen" in feedback.lower() 
