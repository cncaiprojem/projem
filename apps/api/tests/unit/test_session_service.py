...
class TestSessionService:
    ...
    def test_create_session_max_limit_exceeded(self, db: DBSession):
        ...
        # Verify security event was logged
        limit_events = db.query(SecurityEvent).filter(
            SecurityEvent.user_id == user.id,
            SecurityEvent.type == "SESSION_LIMIT_EXCEEDED"
        ).all()
    ...
    def test_rotate_session_success(self, db: DBSession):
        ...
        # Verify security event was logged
        rotation_events = db.query(SecurityEvent).filter(
            SecurityEvent.user_id == user.id,
            SecurityEvent.type == "SESSION_ROTATED"
        ).all()
    ...
    def test_rotate_session_invalid_token(self, db: DBSession):
        ...
        with pytest.raises(SessionSecurityError) as exc_info:
            self.service.rotate_session(
                db=db,
                current_refresh_token="invalid-token",
                ip_address="192.168.1.100"
            )
    ...
    def test_rotate_session_reuse_detection(self, db: DBSession):
        ...
        reuse_events = db.query(SecurityEvent).filter(
            SecurityEvent.user_id == user.id,
    ...
    def test_rotate_session_device_mismatch(self, db: DBSession):
        ...
        session, token = self.service.create_session(
            db=db,
            user_id=user.id,
            device_fingerprint="original-device",
            ip_address="192.168.1.100"
        )
        ...
        device_mismatch_events = db.query(SecurityEvent).filter(
            SecurityEvent.user_id == user.id,
            SecurityEvent.type == "SESSION_DEVICE_MISMATCH"
        ).all()
    ...
    def test_revoke_session_success(self, db: DBSession):
        ...
        # Verify security event was logged
        revoke_events = db.query(SecurityEvent).filter(
            SecurityEvent.user_id == user.id,
            SecurityEvent.type == "SESSION_REVOKED"
        ).all()
    ...
    def test_revoke_session_already_revoked(self, db: DBSession):
        ...
        # First revocation
        result1 = self.service.revoke_session(
            db=db,
            session_id=session_id,
            reason="first_revoke"
        )
        ...
        result2 = self.service.revoke_session(
            db=db,
            session_id=session_id,
    ...
    def test_revoke_all_user_sessions(self, db: DBSession):
        ...
        # Verify security event was logged
        bulk_revoke_events = db.query(SecurityEvent).filter(
            SecurityEvent.user_id == user.id,
            SecurityEvent.type == "ALL_SESSIONS_REVOKED"
        ).all()
